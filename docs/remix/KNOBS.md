...
| `RPCS3_REMIX_RETRYUNSUP` | `1` (on) | Let the albedo unit walk step past a unit `bind()` refused for a permanent format reason instead of stopping on `FPALBEDO`'s guard. That guard prevents a normal map being substituted for a diffuse map, which cannot be what is happening when the unit holds nothing bindable. Haze puts a 2048x2048 DEPTH16 shadow map on the lowest referenced unit of every shadow-receiving draw and names no albedo unit in its ucode, so the guard fired unconditionally and 55,360 draws reached Remix untextured. `0` restores the unconditional guard. Counter: `tex_retry_unsupported`. | |
