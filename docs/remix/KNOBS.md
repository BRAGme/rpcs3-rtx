...
| `RPCS3_REMIX_CAMRELATCHTOL` | `0.8` | **How far the mid-frame relatch may reach for "this frame's version of the active camera" (round 52).** `0.8` is `s_camera_discontinuity_tolerance`, i.e. exactly the test `consider_camera_candidate` applies today, so the default is **bit-exact** and the knob can only narrow. **The menu symptom is this relatch, not the vote.** It refreshes `m_active_camera` from the *first* candidate of the frame merely within tolerance of the active view, and on Eat Lead (BLUS30267) run 13 that was, in every sampled main-menu frame, a **one-vote animated part** -- one of four near-identical `633ba74c6b235af7` clusters drifting `-2.3212` -> `-2.3270` -> `-2.3329` frame to frame -- while the **9-vote static winner** `001e6e3d1495ee54` sat at `[-1.6456 2.4655 -0.5667]`, `view_delta=0.40` away. `submit_camera()` submits `m_active_camera` *after* that refresh, so the winner was >= 0.05 from what was actually submitted on **727 of 1,074** contested menu frames (68%) and 1,312 of 4,731 in gameplay (28%) -- the camera riding the character's revolver. The rasterised picture was never wrong (`world = fused x ref^-1` is exact whichever cluster is the reference); the world pose, the lighting anchor and the temporal history were. `0.1` is the Eat Lead value: 4x under the props' 0.40 and >= 3x over the true camera's own per-frame motion (|delta| p50 0.054 / p90 0.17 / p99 0.33 units against a matrix norm of ~8-35; 0 / 0.002 / 0.17 in the menu). A frame whose own cluster exceeds it simply does not relatch -- today's flip-only latch for that frame. Counter `cam_relatch_refused` (candidates that pass the 0.8 test and fail this one), structurally 0 at the default. `env_float` rejects <= 0, so this cannot disable the relatch; `CAMRELATCH=0` is that switch. Risk: too tight during fast turns leaves `cam_relatch` well below one per elected frame with `world_ref_stale` spiking and `cam_relatch_refused` flat -- go to `0.2`. | |
...
| `RPCS3_REMIX_CAMSTICKY` | `0` (off) | **Identity hysteresis in the no-lock vote (round 52).** A cluster that is the active camera's **own cluster** -- the mid-frame relatch's predicate, evaluated at `CAMRELATCHTOL` -- outranks any cluster that is not, regardless of votes; among equals the observation count then the score decide exactly as today; and a frame with **no** own cluster elects by vote exactly as today, so a real cut still goes through the existing 12-frame confirm. This is the lock path's own `continuous_with_active_primary` rank generalised to the path a title with no usable lock actually takes; the lock path is untouched, and nothing here can elect a cluster the vote could not already have elected -- it only reorders. **Measured on Eat Lead run 13:** 72 identity changes in 4,806 gameplay frames (15 per 1,000, winners 10-25 units apart, e.g. frames 4515 -> 4528 -> 4573 -> 4590), of which **54** happened while a cluster continuous with the previous winner (view <= 0.8, same lens) was present in the frame and simply lost the vote by a mean margin of **2.3 observations**; 42 while one within 0.1 was present. Lens changes degrade gracefully: the aim-down-sights sweep 35.983 -> 28.120 moves the projection ~0.3% per frame, under the 1e-2 projection tolerance, so the incumbent follows it, and only the sweep's first step (35.983 -> 35.307, delta ~0.0125) exceeds it -- for one frame, in which sticky falls back to the vote, which is today's behaviour and a frame today's relatch already skips. **Arm only with `CAMRELATCHTOL` below `0.8`:** with the relatch still refreshing from props, "own cluster" would be measured against a prop-relatched view and the hysteresis would lock onto the prop -- the tell is `camsticky=1 camrelatchtol=0.8` on the banner. Counters `cam_sticky_kept` (an own cluster elected over a cluster with strictly more votes -- the flip-flop that did not happen) and `cam_sticky_lost` (the frame had clusters and none was the incumbent's own; 48 gameplay frames of the baseline were in that state, and whether those are cuts or one-frame gaps is what decides if a sticky *hold* is worth having). `0` restores the plain vote bit-exactly. | |
| `RPCS3_REMIX_CAMTRACK` | `0` (off) | **Continuity of camera identity over TIME (round 53) -- the mechanism `CAMSTICKY` could not be.** A bounded set of camera *tracks* is maintained at flip from the frame's cluster census: the incumbent track keeps the elected identity for as long as its object is drawn; while it is unmatched for up to `CAMTRACKMISS` frames the frame's candidate is discarded and the existing latch holds `m_active_camera`; when it dies, the matched track with the greatest **tenure** is elected (ties: the frame's vote winner, then cumulative votes, then score -- so a fresh scene reduces to today's vote). **Why no ranking rule can work here.** Read with a position-continuity tracker, Eat Lead run 13's 83,257 cluster rows say the winner track changed **84 times in 4,761 gameplay frames** and **in 80 of the 84 the previous winner's track was ABSENT from the frame** (89 of 91 with the tolerance widened to 2.5 units). Only **4** were an incumbent present and out-voted -- the case `CAMSTICKY` addresses -- which is why arming it did not move the rate (46 `cam-elect` rows in frames 6730-9920, 14.4 per 1,000 against run 13's 15, while `cam_sticky_kept=238` says the rank fired). Every cluster is `split(M_obj x V x P)`, a placed object's basis with the true view folded in; **no cluster is the bare `V x P`** and the most persistent track is drawn in only **1,466 of 4,761 frames (30.8%)**. There is no true-camera cluster to prefer, only a choice of WITNESS OBJECT, and the fault is that the witness is re-chosen whenever it is culled. The winners are provably static: light `878edd2ba07c8089` sits at exactly `[-11.81 -7.136 1.844]` whenever `1b20d02263aa8ee4`'s cluster wins (frames 7851, 7988, 8933, 9085 -- over 1,200 frames of player motion) and at exactly `[-3.219 12.5 1.274]` under `7565b4cbd93682d9`. **Simulated on the logged clusters: 16 handovers in 4,761 frames (3.4 per 1,000) against 84**, incumbent duration p50 252 / p90 827 / max 1,021 frames; the vote disagreed with the tenure incumbent on **55.7%** of frames, which is why this is a knob with counters and not a tweak of the rank. Implemented by rewriting `m_frame_candidate` at flip (the `CAMFBRELATCH` shape), so the source-change test, the 12-frame confirm, the age, the hold and every counter below still run; it reorders nothing in `consider_camera_candidate` and cannot elect a cluster the census did not contain. **Scoped off whenever a camera lock is configured.** Under `CAMTRACK`, `cam_sticky_kept` / `cam_sticky_lost` describe **the vote**, not the elected identity. Counters `cam_track_kept`, `cam_track_override` (the share the rule *decided* -- simulation 55.7%; below 5% means it is not deciding, check the banner and `camlock`), `cam_track_missed`, `cam_track_handover`, `cam_track_reset`, `cam_track_expired`, `cam_track_overflow` (must read 0, else raise `s_max_camera_tracks`). Census `Remix cam-track:`, one line per handover / reset / incumbent expiry; `Remix camera trace:` gains `track=` `tenure=` `handover=` and `Remix guest-light-drift:` gains `cam_track=`. `0` restores the vote bit-exactly. | yes |
| `RPCS3_REMIX_CAMTRACKPOS` | `1.0` | How far a cluster's implied eye may move in one frame and still be the same witness object. **1.0 world unit is 1.35x the largest per-frame step of any long track in run 13** (p99 0.12-0.28 depending on the track, max 0.74) and at least **10x under the 10-30-unit separation of 47 of the 84 winner jumps**, so it survives a flick turn without ever letting a handover pass as continuity. The `camera_own_cluster` identity keys (`surface`, `target`, `clip`, `archetype`, `has_reference`, `group_count`) must match too, and the coarse `s_camera_discontinuity_tolerance` matrix gate is applied first so a candidate whose `mat4_invert` failed and left `position` at the origin cannot match a distant track. `vp_hash` is a preference among clusters inside the tolerance, never a key: one moving viewpoint's first-observed program alternates between frames. **Too tight** shows as `cam_track_missed` bursts and a `cam-track` expire followed by a handover on the same `inc=` id within four frames, usually with a `Remix timing:` frame over 60 ms -- go to `2.0`, still >=5x under the base separation. **Too loose** lets a track hop between stacked instances 0.13-1.2 units apart (frame 4526's `9892c73c...` triple at 0.13 spacing); the tell is drift rows with `0.25 < d < 1.5` at `cam_age=0` under an unchanged `cam_track` id and no `cam-track` row -- go to `0.5`. `env_float` rejects values <= 0. | |
| `RPCS3_REMIX_CAMTRACKMISS` | `3` | Consecutive frames the incumbent's track survives with no matching cluster before it dies and a handover happens. During those frames the frame's candidate is discarded and `m_active_camera` is held, so `cam_track_missed` and `cam_held` count the same frame. **3 bridged 14 of the 16 simulated handovers; 8 bridged 11 of 13** -- the extra five frames buy nothing and lengthen every hold, and a held camera is exactly the state in which the injected lights slide (`guest_light_drift_stale`). It also bounds the co-presence window a `CAMREBASE` bridge may reach back through (`miss + 1` frames). | |
| `RPCS3_REMIX_CAMREBASE` | `0` (off) | **Carry the basis across a handover (round 53). Requires `CAMTRACK`.** A persistent rigid transform `R` (submitted view = `R x raw view`) is composed at each bridged handover from the relation the outgoing and incoming witness expressed while they were **co-present**: for two static witnesses `C = V_new x V_old^-1 = M_new x M_old^-1`, in which **the true camera cancels**, so it is measurable in any frame where both are drawn. `reference_inverse` and `view_proj_inverse` are post-multiplied by `R^-1` at the same time, so `world' = fused x ref_inv x R^-1` and `world' x (R x V) x P = fused` -- the composite the whole backend rests on stays exact -- and `position` is re-derived from the rebased view. All continuity predicates (`camera_own_cluster`, the three trace `view_delta`s, the cluster `winner` flag) compare **raw** views, while the flip's source-change / discontinuity / pending tests stay on the submitted ones: both sides are rebased by then, so a bridged handover reads as continuous and the 12-frame confirm stops firing on it, which removes ~12 stale frames per handover as a side effect. A bridge whose self-check residual exceeds `s_camera_pending_match_tolerance` (0.5) is refused and `R` reset (`cam_rebase_refused`); a handover with no co-present predecessor resets `R` to the identity (`cam_rebase_reset`) and falls through to today's confirm. `R`'s 3x3 is re-orthonormalised (Gram-Schmidt on the rows) at every composition, and `rdet` on the census line is its determinant -- drift away from 1 is the tell that the composition is accumulating split noise. **Mode `2` -- handing over away from a witness that is itself moving -- is DOCUMENTED AND NOT IMPLEMENTED;** this round only measures it, via `cam_track_witness_moving` (a witness whose relation to the incumbent drifted past `s_camera_witness_tolerance` = 0.05, printed as `cam_witness_moving`) and `cam_track_incumbent_moving` (two or more witnesses drifted in one frame, so the common term is the incumbent; printed as `cam_incumbent_moving`). **Preconditions.** `gauge_absent` must be 100% on the title: the two gauge-path writes of `m_active_camera.view` (the `GAUGECAMHOLD` restore and the anchor-split install) are instrumented with `cam_rebase_bypassed`, which must read 0 -- non-zero says the gauge path has to be taught the rebase before this can be armed there. **Absolute-world consumers are not adjusted:** under `R` the "world" Remix sees is the first witness's basis composed with the bridges, so an authored-light placement (`AUTHOREDLIGHTS`, off) would have to include `R^-1`. The split-failure recovery candidate is deliberately left alone because it comes out raw by construction -- `relative` is built from `reference_inverse`, which carries `R^-1`, while `m_active_camera.view` carries `R`, and the two cancel. Counters `cam_rebase_applied`, `cam_rebase_refused`, `cam_rebase_reset`, `cam_rebase_bypassed`. `0` restores the un-rebased camera bit-exactly. | |
| `RPCS3_REMIX_GLDRIFTMAT` | `0` (off) | **The raw matrices behind every guest-light drift row (round 54). MEASUREMENT ONLY -- nothing in this round moves a matrix or changes a submission.** A drift row says the light moved; as of round 54 it also says, via `ref_same`, whether the DIVISOR (`m_active_camera.reference_inverse`) is bit-identical to the one the light was last placed under. When it is identical and the light moved anyway, two causes remain -- the guest moved the fixture, or the decode wobbled -- and they are told apart by the SHAPE of the change: a guest move is a translation row only, a decode wobble is spread through the entries. At `1` a second line is emitted immediately after each drift row: `Remix guest-light-drift-mat: id= frame= fused=[16 x %a] ref=[16 x %a] fused_valid= world_branch= line=n/64`. `%a` because narrowing to `%g` destroys exactly the residual being looked for (the `Remix pick-deep:` precedent). `fused` is `m_ref_pick_fused`, the current draw's own folded matrix, written on `per_draw_transform`'s `has_reference` branch; only the VALIDITY FLAG is reset per call, so on `fused_valid=0` the fused block is some earlier draw's matrix and must be discarded -- `gl_mat_unavailable` counts those and `world_branch` names which reference the draw actually divided by. The `ref` block is always this frame's. Shares the drift census's 64-per-window cap, so an armed run cannot flood the dump. Clamped to `1`. `0` restores today's output bit-exactly. | |
| `RPCS3_REMIX_CAMRELATCHTRACE` | `0` (off) | **Census of the MID-FRAME RELATCH (round 54). MEASUREMENT ONLY -- the relatch rule, its tolerance and its effect are untouched.** The relatch is the only writer that refreshes `m_active_camera` between the flip latch and a draw, so it decides which draws of a frame divide by a fresh reference and which by the previous flip's (`world_ref_stale` is 7.2% / 10.6% of world draws in the two logged runs) -- and because it takes the FIRST candidate of the frame inside `CAMRELATCHTOL` rather than the witness tracker's incumbent, it can also refresh from a DIFFERENT OBJECT than the one the tracker is following. Neither fact was logged. At `1` two rows are emitted, sharing one 64-per-window budget: `Remix cam-relatch:` (one per relatch -- the raw eye it took, `step` = how far it moved the SUBMITTED eye, `inc_d` = its distance from the incumbent track's raw eye, `inc_ok` = whether that was inside `CAMTRACKPOS`, and `ord` = world draws already placed on the stale reference this frame) and `Remix cam-relatch-disagree:` (one per frame in which the relatch's draw and the tracker's own match for the incumbent are more than **0.05 units** apart -- a READING THRESHOLD, `s_camera_witness_tolerance`'s magnitude and ~3x the p50 per-frame eye step, not a gate). The two quantities are deliberately measured in different bases: `step` submitted-vs-submitted so it is comparable with a drift row's `d`, `inc_d` raw-vs-raw because the tracker stores raw cluster eyes -- mixing them would read as the translation magnitude of `R` on every row after a `CAMREBASE` bridge. Counters `cam_relatch_incumbent` / `cam_relatch_foreign` / `cam_relatch_noinc` partition `cam_relatch` EXACTLY (the acceptance check) and `cam_relatch_agree` / `cam_relatch_disagree` partition the frames that had both a relatch and a matched incumbent; all five accumulate whether or not this is armed, so the knob only decides whether the rows are printed. `foreign` or `disagree` climbing, with drift rows whose `d` equals the disagree `d`, is the finding that the relatch is refreshing from siblings -- an identity fault, not a tolerance one. `0` restores today's output bit-exactly. | |
| `RPCS3_REMIX_CAMREFSYNTH` | `0` (off) | **Refresh the un-projection REFERENCE on a tracker miss without touching camera identity (round 55). Requires `CAMTRACK`.** When the tracker's incumbent witness is alive but not drawn, the flip discards the frame candidate and keeps the whole of `m_active_camera` -- identity *and* matrices. Freezing the identity is right; freezing `reference_inverse` is what makes the world and the injected lights slide by the camera's own motion for the length of the hold. Round 54's rows say the **holds end in a basis SWITCH, not a thaw**: nine of eleven hold-ending snaps were a handover, a reset or a confirm installing a different witness's basis, and under `CAMTRACK` the 12-frame discontinuity confirm can never restore the outgoing witness (it is dead -- that is why the handover happened), so the confirm is pure delay. Two static objects express a rigid relation the true camera cancels out of, `rel = view_J x view_I^-1 = M_J x M_I^-1`, already stored as `track.rel_first` / `rel_last`; so a frame where I is missing and J is drawn still places in I's frame: `view_I = rel^-1 x view_J`, `ref_inv_I = ref_inv_J x rel`, and `world = fused x (ref_inv_J x rel) = M_obj x M_J^-1 x M_J x M_I^-1 = M_obj x M_I^-1`. **`1`** synthesises the incumbent's candidate from the best qualifying co-present witness on a `cam_track_missed` flip and installs it through the ordinary latch (so `latch_frame` is stamped and the hold's `new_age` can never reach 2), **and** makes the mid-frame relatch refuse a candidate outside `CAMTRACKPOS` of a live incumbent (`cam_relatch_foreign_refused`) -- on a synthesised frame every near candidate is a *different* object, the mechanism that hijacked the basis at run-54 frame 1568. **`2`** adds: a flip whose candidate was elected by a tracker handover/reset skips the confirm (`cam_ref_synth_install`), so the switch lands at the handover frame instead of 12 flips later -- more switches, no confirm-window slide. A synthesis that cannot be made **holds exactly as today** and increments a named reason counter; never a guessed matrix. Partition: `cam_ref_synth + cam_ref_synth_nowitness + cam_ref_synth_short + cam_ref_synth_rigid + cam_ref_synth_basis + cam_ref_synth_badrel == cam_track_missed` while non-zero (all six 0 while it is 0); the refusals are, in test order, *basis* (the active camera is not in the incumbent's basis), *nowitness*, *short* (one co-present frame, no rigidity test possible), *rigid* (`rel_last` vs `rel_first` beyond `s_camera_witness_tolerance` -- the witness was moving) and *badrel* (non-invertible, or the bridge's own self-check failed). Identity is not written: `m_camera_track_incumbent`, `track.position/view/missed`, `track.moving`, expiry and the tenure election are untouched, and the `moving` verdict stays gated on `CAMREBASE` so the election is bit-exact on this arm. Tells: `Remix cam-synth:` rows (under `CAMRELATCHTRACE`) carrying `rigid`/`err`/`step`; `refsrc=` and `switch=` on every guest-light-drift row; `refused=` on every cam-relatch row; `cam_basis_switch`, `cam_discontinuity_held` and `cam_switch_confirmed` on `Remix live:`. **`cam_held` changes meaning on this arm** -- a synthesised flip is not a hold, so `cam_track_missed` and `cam_held` stop counting the same frame (contrast the `CAMTRACKMISS` row). This knob acts only at misses and can never cancel a basis switch; **`CAMREBASE` is the complementary mechanism** -- it acts only at handovers (23 of run 54's 28 switches were co-present handovers) and cannot reach the 71 missed flips. They share one relation table and compose. `0` restores today's output bit-exactly -- the measurement fields above print on both arms. | |
| `RPCS3_REMIX_CAMXFLIP` | `0` (off) | **Un-mirror the SUBMITTED camera's right vector (round 56). Image-invariant; fixes the runtime's free camera.** `try_split_once` does not *recover* the view, it **constructs** it: `forward` and `up_hint` are unprojected out of the fused matrix and carry no convention, but the basis is then closed with `right = up_hint x forward` and `up = forward x right`, and that **cross-product order assumes the guest's world is left-handed**. Eat Lead's is right-handed, so the constructed row 0 points screen-**LEFT**, and the recovered `P = viewToWorld x fused` absorbs the error as a negative x-scale: the product `V x P` is right while **neither factor is**. **MEASURED with no rebuild**, off `score_perspective`'s own `-0.5` dock for `m[0][0] < 0` (which makes an integer score impossible once it fires): `Remix camera trace:` rows read 8167 x `score=5.500` + 672 x `score=4.500` on run pid27232 and 8475 x `score=5.500` on run pid30572 -- **zero integer scores in 17,314 resolved frames**, so `P[0][0] < 0` on 100% of them (and `P[1][1] > 0` on all of them too, since the `-1.0` dock would have restored an integer). **A determinant test on the view clears this and is wrong:** `right = up x forward` makes `(right, up, forward)` orthonormal with `det = +1` **by construction** on every frame whatever the world's handedness -- the round-20 viewmodel trap again -- which is why `vdet=` reads 1 on both arms and `p00=` is the field that names the bug. **The symptom:** `RtCamera::getRight()` is `viewToWorld[0].xyz()` verbatim, the runtime's free camera strafes along it (`currentPosition += moveLeftRight * freeCamViewToWorld.data[0].xyz()`) and ties yaw to the same row, while pitch reads row 1 and forward/back row 2 -- so a reversed row 0 reverses left/right for **movement and turning and nothing else**, exactly as reported. **`1`** inserts `S = diag(-1,1,1,1)` between the submitted `V` and `P` in `submit_camera` and nowhere else: `V' = V x S` negates the view's **column** 0 (all four rows, translation included) and `P' = S x P` negates the projection's **row** 0, so `V'P' = V S S P = VP` **exactly** -- verified bit-identical in f32, `max|diff| = 0`. `remixapi_SetupCamera` applies the matrices verbatim, primary rays come from the inverse of the *product*, and `determineInstanceFlags`' winding test reads `viewToProjection x worldToView` -- also the product -- so **the rendered image cannot change**. What does change: `viewToWorld'` row 0 is the true screen-right and its 3x3 determinant is `-1` (Remix's convention for a right-handed world under a left-handed projection), and `P'[0][0] > 0`. `P[2][2]`, `P[3][3]`, near, far and fov are untouched, so MathLib's `bLeftHanded = a22 > 0` still reads Left-handed and only the dev menu's `Overall Handedness` (`isLHS ^ isMirrorTransform(viewToWorld)`) flips: **`Projection Handedness: Left-handed` / `Overall Handedness: Right-handed`** is the acceptance reading, together with an unchanged image. **Boundary-only by construction:** applied after both `to_camera_matrix` copies, so `m_active_camera` is never written and the split, the tracker, the relatch, the rebase, the reference synthesis, the drift census and every light placement never see it -- and every internal quantity is S-invariant anyway (`view x projection = fused`, `reference_inverse = inverse(folded)`, `rel = view_J x view_I^-1`, and `position` = row 3 of `inverse(view)`, which survives because S negates only column 0 of `V` so `inverse(V') = S x inverse(V)` keeps row 3). `CAMTRACK` / `CAMREFSYNTH` / `CAMREBASE` therefore cannot regress from it; every census row keeping its shape on the armed arm is the pre-registered refutation. The sky and viewmodel twins copy `camera_info` below the fix and inherit it, and the twin's `std::copysign(x_scale, camera_info.projection[0][0])` then copies a **positive** sign. Counters `cam_xmirror` (submitted `P[0][0] < 0`, counted on **both** arms) and `cam_xflip` (the flip was applied); pre-registered reading `cam_xmirror == cam_xflip == cam_resolved` armed, `cam_xflip == 0` on the control. Fields `vdet=` / `p00=` at the end of every `Remix camera trace:` row, also on both arms. **NOT the root cause:** `try_split_once`'s cross-product order is where the mirror is born, but every continuity predicate, relation table and rebase in three shipped rounds compares views produced by that function, so changing it there moves internal state on every frame and cannot be A/B'd at the boundary. `0` restores today's output bit-exactly. | |
| `RPCS3_REMIX_PICKFRESH` | `0` (off) | **Measure the POSE ERROR of a followed object, not just its per-frame delta (round 57). MEASUREMENT ONLY -- nothing on the submit path changes.** The user-visible defect is static props visibly wobbling with camera movement; the `Remix pick-follow:` census sees it as a 0.05-0.25-unit band on `d_origin` that is 3-10x the camera's own translation step on the same frames. **The mechanism, MEASURED on the frozen pid-28088 slice before any of this was built.** A world draw is placed as `world = OS x (fused x reference_inverse)`, and the frame's own `reference_inverse` only exists once the frame's first camera-carrying draw reaches `consider_camera_candidate` and the mid-frame relatch installs it -- so every world draw before that ordinal is divided by the **flip latch of frame N-1** and lands at `OS x M x Q_N` with `Q_N = V_N x V_{N-1}^-1`, the camera's own frame-to-frame rigid motion. A turn of `dtheta` about the eye displaces a prop at eye distance `r` by about `r x dtheta`; a translation `t` of the eye displaces it by `-t`; a draw arriving **after** the relatch cancels exactly (`fused x (V_N P_N)^-1 = M`) and cannot move at all. In the slice, the two props that visibly swim are `age=1` on **298 of 298** rows and 116 of the run's 122 band rows are theirs, while the three quiet objects are level chunks at the world origin and `age=0` on 93% of theirs; those props print `d_origin=0` **exactly** on the frames whose `cam-relatch` row reads `step=0`, which excludes precision, quantisation and genuine guest motion outright. Run-wide the pre-relatch population is `world_ref_stale=207124` of 3,087,767 world draws (6.7%, ~12.5 per frame). **What the knob adds.** `per_draw_transform` stashes the three operands the live path actually used (`fused`, the object-space composite obtained by running its own `prepend_object_space` lambda on an identity, and `*reference` -- **the divisor**, not the member); at flip, after `flush_deferred_at_flip()` and **before** `apply_gauge_anchor_camera()` overwrites `m_active_camera`, that same `fused` is re-divided by the frame's own relatched reference and one `Remix pick-fresh:` row is written beside the frame's `Remix pick-follow:` row with `origin` / `origin_fresh` / `err` / `err_v` / `first` / `d_fresh` / `r` / `cam_step` / `cam_rot` / `recheck` / `fresh` / `ord` / `relatch_ord`. **`recheck` is the built-in proof:** re-dividing by the SAME stale reference must reproduce the submitted origin **exactly** -- same operands, same order, same precision -- so a non-zero `recheck` means an operator runs between the divide and the submit that the stash does not carry, and `err` must not be read at all (`pf_recheck_fail` counts any row over `1e-5`). The second built-in control is age: on an `age=0` draw the stale and the fresh reference are the same matrix, so `err` must be **exactly 0**. Five fields (`age`, `ord`, `relatched`, `d_rot`, `cpos`) are appended to the `pick-follow` row on **both** arms, because a reader should not have to infer the reference age by subtracting two frame numbers; `d_rot` is the row-normalised 3x3 delta, the half `d_basis` structurally cannot see (row **norms** are invariant under every rigid transform, which is why `d_basis` reads 1e-9 beside `d_origin` 0.2 -- that pair rules out every *non-rigid* mechanism and says nothing about rotation). Counters `pf_rows` / `pf_first` / `pf_stale` / `pf_fresh` / `pf_step0..4` / `pf_age_unknown` / `pf_rot_degenerate` / `pf_fresh_rows` / `pf_norelatch` / `pf_recheck_fail` / `pf_place_refused` / `pf_err0..4` on `Remix live:`, in the `gl_step` buckets verbatim so props and lights read side by side. **Three partitions are the acceptance check, not a hope:** `pf_stale + pf_fresh == pf_rows`, `pf_first + pf_step0..4 == pf_rows`, `pf_norelatch + pf_place_refused + pf_err0..4 == pf_fresh_rows`; any mismatch means the instrumentation is wrong and the census must not be read. **Pre-registered readings:** (R1) `d_fresh` on `age=1, fresh=1` rows of the small props has median <= 1e-3 and p90 <= 5e-3 -- the prop is static under the frame's own reference, and a failure kills the mechanism outright; (R2) on the same rows with `err > 0.05` the two-sided bound `\|r x cam_rot - cam_step\| - 0.02 <= err <= r x cam_rot + cam_step + 0.02` holds on >= 90%; (R3) still control -- rows with `cam_step < 1e-3` and `cam_rot < 1e-4` have `err < 0.01` on >= 95% and `d_origin = 0` exactly on >= 90%; (R4) negative control -- every `age=0` row has `err == 0` exactly; (R5) `pf_err1+pf_err2+pf_err3 > pf_step1+pf_step2+pf_step3`, i.e. the visible pose error is larger and more frequent than its per-frame change, which is why the band reads several times the camera's translation step. The 0.25 drift-census threshold is **not** moved, duplicated or lowered by any of this -- it is the row threshold of a capped census and the whole round-54/55/56 decision table is defined on the population above it; the band belongs in counters, which is exactly what `gl_step1` already is for lights. `0` restores today's output bit-exactly. | |
| `RPCS3_REMIX_DEFERPRERELATCH` | `0` (off) | **The FIX for the mechanism `PICKFRESH` measures: hold a world draw that would be divided by a STALE reference and flush it at the mid-frame relatch, re-divided by the fresh one (round 57). SHIPS DARK -- it changes submit order.** Read the `RPCS3_REMIX_PICKFRESH` row above first; this knob assumes its arithmetic. **Why it is a fix and not a prediction:** the held draw is re-placed with exactly the arithmetic a draw arriving *after* the relatch would have used -- `world = OS x (fused x reference_inverse)`, normalise, `to_remix_transform` -- so there is no extrapolation and no residual by construction. Extrapolating the reference instead (`Q_pred = V_{N-1} V_{N-2}^-1`) is a guess where this is exact, and is deliberately **not** what this does. **The machinery already exists and is already proven:** `deferred_instance` / `submit_deferred` / `flush_deferred_for_anchor` / `flush_deferred_at_flip` were root-caused, shipped and **confirmed by the user in-world** to stop props sliding on Haze, then switched off for cost (MEASURED ~14 fps at ~50 held instances per frame, of which 72% did no work at all). On this title `DEFERPREANCHOR` is armed but **structurally inert** -- `defer_absent_declined=3087767`, `defer_buffered=0` -- because it keys on gauge anchors this title never produces. This knob keys on the **relatch** instead: the population is `world_ref_stale`, MEASURED at **207,124 of 3,087,767** world draws (6.7%, about 12.5 per frame, against a relatch-ordinal median of 12), and every held draw does useful work. **Where it acts:** `per_draw_transform`'s `gauge_anchor_absent` branch arms `defer_candidate` when `camera_relatch_enabled() && m_active_camera.valid && latch_frame != m_frame_counter`, **before** the `DEFERPREVONLY` decline, so `defer_relatch_armed + defer_absent_declined == gauge_anchor_absent` stays a partition; the buffering site copies the flag onto `deferred_instance::waits_relatch`; `flush_deferred_for_anchor` skips those entries however well their render-source key matches (they are waiting on a different event); and the new `flush_deferred_for_relatch()` runs inside `consider_camera_candidate`, in the `!foreign_refused` arm immediately after `++cam_relatch_midframe`. Leftovers on a frame that never relatched go out through the existing `flush_deferred_at_flip` with the transform they carry -- today's behaviour one flush later, so **no draw is ever lost**. A held draw carrying a viewmodel operator keeps its immediate transform and is counted `defer_relatch_vmop` (structurally 0: the viewmodel branch clears `defer_candidate` upstream). **What changes, and why it therefore ships off:** submit **order**. Those ~12 instances a frame leave their draw ordinal and go out at the relatch ordinal. Nothing else about them changes -- same `InstanceInfo`, same pNext chain, same picking value -- but a title that depends on submission order for decals or blended layers could show it, and that is not decidable from a log. So it is an **A/B**: run `PICKFRESH=1` alone, read the verdicts, then repeat the same protocol with this armed. Counters `defer_relatch_armed` / `defer_relatch_flushed` / `defer_relatch_vmop`, with partitions `defer_relatch_armed + defer_absent_declined == gauge_anchor_absent` and `defer_buffered == defer_fresh + defer_relatch_flushed + defer_flip`. **The within-run proof** that the runtime received the fresh transform is on the `pick-fresh` row: `deferred=1` with `d_deferred=0` **exactly**, where `d_deferred` is the L1 between the transform this flush rebuilt and the `origin_fresh` the instrument computed independently at flip from its own stash -- two paths to one number; `pf_deferred_mismatch` counts any row where they disagree and must be 0. **Pre-registered acceptance on the fix arm:** `defer_relatch_flushed >= 0.9 x defer_relatch_armed`; `deferred=1` on >= 90% of the small props' `age>=1` rows with `d_deferred == 0` exactly; `pf_deferred_mismatch == 0`; R1-R4 from the `PICKFRESH` row **unchanged** (the instrument is upstream of the fix); and the user reports the small props *steady* while walking and turning, with no slowdown. **Tell it went wrong:** flicker or missing decals only on this arm, or a reported frame-rate drop -- disarm, and `0` restores today's output bit-exactly. | |
...
| `RPCS3_REMIX_SPLITROWS` | `1` (on) | **A matrix row that never lands in the lane the matcher watches (round 57).** `match_dp4_chain` wants four DP4s writing four lanes of one register. Ratchet & Clank Collection (BCUS98282, RC1) gives it two: replayed with `docs/remix/vpdis.py` from `646DEC336DEBA29A.vp`, the W row is computed into a DIFFERENT temp lane (`11: DP4 r0.x <- r0, c[3]`) and reaches `r1.w` through a DUAL-WRITE MOV (`14: MOV o[10](TEX3)=r1.w <- r0.xxxx`), while the X row is overwritten by `15: MAD r1.x <- r0.wwww, c[17].xxxx, r1.xxxx`. `c[0..3]` is the view-projection and `c[4..7]` the object matrix, so the chain behind it is ordinary -- but the program reported `arch=unknown` / 'no matrix chain into HPOS', no camera could be derived, and EVERY world draw was refused: **world_refused = 5,967,184 = world_fallback, cam_resolved=0, submitted=0**, a black screen behind a runtime otherwise healthy at 59 FPS with textures loading. After: `arch=fused`, `world_applied=18,137,400`, `world_refused=9,911` (0.05%). `repair_split_rows` is the same shape as `repair_wbuffer_z` above -- it rewrites the writer list so each lane names the DP4 that computed it and re-runs the **unchanged** matcher -- and accepts exactly two hops: a `MOV dst.c <- rN.<broadcast>` whose definition is a plain DP4/DPH, and a `MAD dst.c <- a, b, rN.<broadcast>` with exactly ONE of a/b a constant, taking the **addend** as the row. That last requirement is what makes the reading 'row + correction' rather than a guess. **The correction is DROPPED**: on RC1 it is `x += (w - c[17].y) * c[17].x`, a depth-proportional horizontal shear (stereo separation or a screen offset), exactly zero whenever `c[17].x` is; the slot is recorded in `hpos_split_shear_const` so a run can say which constant to read before deciding the omission matters. Hooked **last** in `find_chain`, in both the direct arm and the `resolve_writers` indirect arm (RC1 lands on the indirect one -- its HPOS has a single writer, `MOV o[0].xyzw <- r1.xyzw`), and it only fires when a lane actually moves, so it is a strict widening: no program that matches today can move. `0` restores the previous classification of every program bit for bit. | yes |
...
| `RPCS3_REMIX_STREAKGATE` | `128` | How many times the frame's own median world extent a draw's post-transform bounding box may span before `audit_world_extent` refuses it. Measured against the frame's median, so it needs to know nothing about the title's units. `0` disables refusal while retaining the same measurement and census, recording would-be refusals as `wext_flagged_drawn`. | |
| `RPCS3_REMIX_STREAKALLOWVP` + `RPCS3_REMIX_STREAKALLOWFP` | empty | Exempt one measured vertex/fragment program pair from `STREAKGATE`, only when it draws on the previous frame's main clip. The exact clip guard keeps same-pair auxiliary passes behind the normal gate. | |
...
| `RPCS3_REMIX_POSINPUT` | `1` (on) | **The position is not always ATTR0 (round 50).** `match_const_affine`'s MUL arm used to refuse `mul-not-attr0` on `MUL rN.xyz = v<K>.xyz * c[S].<bcast>` whenever `K != 0`, on the premise that ATTR0 is "the one attribute this backend submits as a position". That premise is a fact about the **backend**, not about the guest. With this on, the arm **records** the input index instead of refusing; it travels on `vp_fingerprint::position_input`, and the single site that fills `m_scratch_vertices` (`RemixGSRender.cpp`, `map_attribute(position_input, ...)`) decodes from that attribute. Nothing else about the shape is relaxed — identity xyz swizzle, a constant broadcast factor and the `mul-twice` guard all still apply — so `build_prescale` folds the scale per draw exactly as it already did for ATTR0. The MAD arm (`mad-not-attr0`), the ADD terminal (`add-not-attr0`) and `match_wdivide` **keep refusing**: no replayed ucode justifies widening them. **Eat Lead (BLUS30267) is the title that forced it** — every one of its world programs feeds HPOS from `0: MUL r4.xyz = v2.xyzx * c[465].xxxx` and its sprite/compositor program from `1: MUL r0.xyz = v1.xyzx * c[467].xxxx`, while ATTR0 carries a byte-quantised normal. Submitting ATTR0 as the position turned the whole game into a cloud of 0..254-unit slabs, which is the white screen. Three independent measurements said so before the arm was touched: `Remix sky-census:` `raw=` boxes integer 0..254 on 42 of 62 rows; `gate=wext` skip-census extents median **250.7** (n=80, the diagonal of a 0..254 box); and `areason=mul-not-attr0` on every Eat Lead world program. **Offline sweep over all 137 stored `.vp` programs:** 39 are in the widened family (25 read `v2`, 8 `v1`, 4 `v3`, 2 `v7`; scale slots c[463..467]), 52 stay ATTR0, 46 match neither. Attributed against `bin
emix_dump.log`, **31 of the 39 are Eat Lead** and 2 are Demon's Souls (`14F25071CB2C399F`, `FEDCEEF2535C262B`) — in both of those the `v7 * c[467].x` is the **bone index** fed to `ARL`, not a position, but both are refused `areason=no-chain` in every recorded run, so the MUL arm never reaches them; and in the whole 735,344-line dump the only `mul-*` refusal ever recorded is the 20 Eat Lead `mul-not-attr0` lines. `0` restores the refusal bit-exactly and drives `pos_input` to `0/0`. Counters `pos_input=<programs>/<draws>` on `Remix live:` and `Remix stats:`; census `Remix posinput:` (one line per program: input index, scale slot and component, archetype). Echoed as `posinput=` on the run-start banner and the live knob tail. | |
| `RPCS3_REMIX_VTXNORMAL` | `1` (on) | **Every vertex this backend submitted carried a CONSTANT normal (round 51).** The one site that fills `m_scratch_vertices` wrote `v.normal = {0, 0, 1}` for every vertex of every draw of every title, and had done since M1. Remix shades from the submitted normal, and for a skinned mesh dxvk-remix's skinning pass rotates it by the same blended bone matrix it rotates the position by (`shaders/rtx/pass/skinning.h` reads `srcNormal`, accumulates `mul(bone, normal) * weight` and renormalises), so the constant was a flat-shaded lie on every curved surface in the scene - least visible on flat walls, most visible on characters, which is where it was reported. With this on, the guest's own normal attribute is decoded (`decode_position`, the same decoder the position uses, so every RSX packing - `cmp` X11Y11Z10, SNORM16, half, float - is handled) and submitted in OBJECT space, which is the space Remix wants on both the rigid and the skinned path. `0` restores the constant bit-exactly. Counters `normals=<programs>/<applied>/<rejected>/<absent>/<degenerate>` on `Remix live:` and `Remix stats:`; census `Remix vtxattr:`. Echoed as `vtxnormal=` on the run-start banner and the live knob tail. | |
| `RPCS3_REMIX_NORMALATTR` | `2` | Which vertex attribute carries the normal. ATTR2 is the RSX fixed-function convention and is the default, but the attribute map is fully programmable and round 50 is the standing lesson that a documented hard-coded index is still an assumption - so this is a knob, and `Remix vtxattr:` is what aims it. **The wrong value cannot smear the scene:** the decode is gated per draw on the MEAN pre-normalisation length of what it read, and a draw outside `[0.7, 1.4]` keeps the old constant and counts `rejected`. A texcoord normalised per vertex would still be unit-length, which is why the gate tests the length the guest STORED rather than the length after normalising. | |
| `RPCS3_REMIX_NORMALCENSUS` | `1` (on) | One `Remix vtxattr:` line per vertex program per run, capped at 64 programs: every attribute the layout feeds that draw, its type and component count, and for the 3- and 4-component ones the min/mean/max decoded vector length over up to 64 vertices, in BOTH readings - `len=` taking the components as signed and `blen=` re-biasing them out of a positive range first (`2x-1`, or `x/127.5-1` for the raw-byte type). Whichever column reads ~1.00 names the slot AND its packing convention. Emitted whatever the verdict was, because the program whose chosen attribute was *refused* is exactly the one whose answer matters. | |
...
| `RPCS3_REMIX_VERTEXBLEND` | `1` (on) | **The transpose of `BONEBLEND`'s shape (round 52).** `match_blend_palette` expresses a rig that blends the palette's matrix **rows** and applies the sum once. Eat Lead (`BLUS30267`) writes the same blend the other way round: it transforms the **point** by each bone -- four indexed MAD column chains, the exact shape of a fused world matrix -- and sums the four transformed points. Mathematically identical, structurally disjoint, and it had no matcher at all, so every arm of `find_chain` refused it by design and the draws landed in `skin_unrec_indexed`. **Measured, round 51: `skin_unrecognised=117183` of which `skin_unrec_indexed=117183`, the other three faults 0, `skin_submitted=0`, `vp_indexed_matched=0 vp_indexed_unmatched=8` -- no character was drawn anywhere in the title.** The eight programs, all replayed instruction by instruction from `bin
emix_ucode`: `001e9e3d5495ee54` `001e6e3d1495ee54` `031ce5a041c06209` `031cb5a001c06209` `a97036b5f4b84f25` `a97006b5b4b84f25` (the six main passes, 52 indexed reads each) and `349bacaa433e003c` `645fc1a8edc0d28d` (the two colour-mask-skipped shadow/depth passes, 16 each). With this on, `match_vertex_blend` runs **last** in `find_chain` -- only on a group every existing arm already refused, so no program that matches today can change matcher or slots -- and delegates each per-bone group to the existing `match_mad_chain_pass(relaxed)`, which already accepts an indexed `.xyz` group at a data-derived stride and already follows the accumulator across a register hop. It also **scopes the whole-program indexed-read audit**: for this family only, the exact `rows x bones` count is replaced by "every indexed read addresses a row of the matched palette through a matched address component". That is required, not a loosening -- these programs rotate the normal (`v0`), tangent (`v5`) and binormal (`v3`) by the same palette's own 256..258 columns into TEX1..TEX6, **36 indexed reads that never touch HPOS**, so the exact count would merely move all 117183 refusals from `skin_unrec_indexed` to `skin_unrec_reads`. `arl_count > 1` and `foreign_reads != 0` stay whole-program and unchanged, and non-vertex-blend chains keep the exact count. `0` restores the refusal bit-exactly: `skin_unrec_indexed` climbs again at ~14 draws/frame, `vp_indexed_unmatched` returns to 8 and the characters vanish. Counters `skin_vertexblend=<draws> vp_vertexblend=<programs>` on `Remix stats:` and `skinvb=<draws>/<programs>` on `Remix live:`; census `Remix: indexed-world` gains `form=vertexblend|matrixblend|single` and `extra=<indexed reads the matched group does not explain>` (expect `form=vertexblend extra=36` on the six main programs, `extra=0` on the shadow pair). Echoed as `vertexblend=` on the run-start banner and the live knob tail. | |
...
| `RPCS3_REMIX_SKIPCMASK` | `1` (on) | **Round 51: refuse, before decode, any draw whose colour mask has all three of R, G and B off** (`NV4097_SET_COLOR_MASK`, index 0). Alpha is deliberately *not* part of the test -- an alpha-only write feeding a later composite still writes nothing the guest sees in RGB. A draw the guest itself masked out of its own colour buffer cannot be in the guest's picture, so submitting it as world geometry can only add surfaces the guest never showed; the condition is title-independent and provable in the guest's own terms, which is why it defaults on. Measured on Eat Lead (`BLUS30267`): `vp=dc76f9c50a96ccba fp=daf4787c7e54c2f7` is ~218 draws/frame, 21 of the 22 `no-unit` census rows, `tex_no_unit=696070` against `tex_none=698200`. Its fragment program is on disk as `bin
emix_ucode$C301C5011EBEE2.fp` (the fp32-export name -- `store_refused_fp_ucode` strips the `0x9e3779b97f4a7c15` term the log adds, so `daf4787c7e54c2f7 ^ 9e3779b97f4a7c15 = 44C301C5011EBEE2`): 48 bytes, three instructions -- `FENCB`, `MOV R0.xyzw, c.x` with `END=1`, and an inline constant `3f800000` -- i.e. **constant 1.0, `sampled=0x0`, no texture unit anywhere**. Widening the albedo walk cannot help a program that samples nothing; any texture it found would be the wrong one. The runtime will not rescue it either: the Remix API instance path reads only the **alpha** bit of `writeMask` (`rtx_instance_manager.cpp`), so a masked draw becomes a fully visible grey surface as soon as `neutral_material` is attached. Counter: `skip_cmask` (`Remix live:`, and `cmask=` on `Remix stats:`); census gate name `cmask` (exempt from `notex_refused_skip`, like `skipvp`). **Denominator shift:** with this on, population A never reaches the albedo walk, so `tex_none`/`tex_no_unit` fall -- that is a smaller denominator, *not* a texture improvement. Read them post-gate. Wrong-direction tell: a missing object with `skip_cmask` non-zero; `0` restores submission in one restart. | |
| `RPCS3_REMIX_SKIPNOTARGET` | `1` (on) | **Round 51: refuse, before decode, any draw whose surface colour target is `surface_target::none`** -- a depth/stencil-only pass with no colour attachment at all. Same argument and the same measured population as `SKIPCMASK`: the Eat Lead pair above is also drawn into `clip=512x512` and `clip=128x128` `target=0` passes with `depth_write=1 blend=0`. There is no colour buffer for those draws to write, so they are generically invisible. Sited before `update_camera_candidate()`, so it also removes them from camera election, which they currently contest (`cam_contested=2351` over 3193 frames). Counter: `skip_notarget` (`Remix live:`, `notarget=` on `Remix stats:`); census gate name `notarget`. **Demon's Souls relabel:** `BLUS30443`'s title-gated `skip_camera_surface` refuses exactly this shape; with this default on it fires first, so that title's count moves from `camsurf` to `skip_notarget`. Identical behaviour, different counter -- `camsurf=0` there is **not** a regression. `0` restores submission. | |
...
| `RPCS3_REMIX_UVINTSCALE` | `4096` | Fallback divisor for S32K (raw 16-bit integer) texcoords, used only where the vertex program's own divisor could not be resolved. `0` submits them raw. **Round 52 correction:** this row used to say the real divisor "is a vertex-program constant this backend does not read", which stopped being true when `UVSCALEUCODE`/`UVAFFINEALL` landed and was actively misleading afterwards -- the backend does read it, and where it still fell back on Eat Lead the cause was the `u8` slot field (fixed, no knob) and the missing MOV-hop shape (`UVSCALEHOP`), not a missing feature. | |
...
| `RPCS3_REMIX_UVSCALEHOP` | `1` (on) | **Round 52.** In `resolve_texcoord_affine`, follow a MOV hop out of the MUL whose scaled operand is a **temp** instead of the attribute -- the exit that used to refuse with `affine:mul-temp-pair` and drop the draw onto `UVINTSCALE`. This is the dominant world shape on Eat Lead (BLUS30267): 595,023 of its 807,360 fixed-divisor draws leave the scalar matcher through `two_attributes`, because that matcher's slice of `o7` is lane-blind and sees both `v4` and `v5`, while the pair that carries (u, v) is fed by `v4` alone. Decoded from the stored ucode, `bin\remix_ucode\70931FA705186608.vp` (the 9,964-vertex world mesh; `1B20D02263AA8EE4.vp` is the same slice with `r2` for `r3` and `c464` for `c463`): `4: MOV r3.zw <- v5.xxxy` / `5: MOV r3.xy <- v4.xyxx` / `6: MUL o7.xyzw <- r3.xyzw, c463.xxxx`, with both constants live at `[0.00048828125 0.00048828125 1 1]` = 1/2048 against the hardcoded 4096 -- i.e. those UVs were submitted **2x too small** and the material was magnified 2x. The hop is MOV-only and stops at the attribute; a hop that lands on a second `MUL(attribute, constant)` is the two-scale family (`8D67DBFCC9DB7A96.vp`: `5: MUL r0.xy <- v4, c464.x` then `11: MUL r0.xy <- r0, c463.x`) and is **refused by name** (`affine:mul-two-scales`) rather than silently reduced to one of its two scales. Counters: `uv_affine_hop`, `uv_affine_two_scales`; read them against `uv_scale_fixed` collapsing. `0` restores the `affine:mul-temp-pair` refusal and the fixed divisor in one relaunch. | |
| `RPCS3_REMIX_UVLANES` | **`0` (off)** | **Round 58.** A vertex program may write the SAME uv pair into `o[7+n].xy` and into `o[7+n].zw` at different scales, and the fragment program's TEX swizzle says which of the two a given texture unit reads. Eat Lead's interior programs do exactly that -- `bin\remix_ucode\7565B4CBD93682D9.vp`: `1: MUL r0.xy <- v4.xyxx, c[464].xxxx` (the 1/2048 dequant) then `10: MUL r0.zw <- r0.xxxy, c[463].xxxx` with **live `c463 = [0.1 0 0 0]`**, i.e. one repeat per ten world units, then `11: MOV o[7](TEX0).xyzw <- r0.xyzw`. `8D67DBFCC9DB7A96.vp` is the mirror image, with the two scales on `.xy`. Two things follow, both behind this one knob: (a) `apply_texcoords` replays the **`.zw` form** (`vp_fingerprint::texcoord_affine_zw`, resolved by the same walker with `lane_pair = 1`) for a unit whose TEX samples `.zw`, instead of applying the `.xy` divisor to it -- which is the ten-times-too-dense tiling; (b) `albedo_unit_mask` drops units sampled through the **macro** lane pair when the mask still holds a wide colour unit on the other pair, never demoting the last wide unit and containment-tested exactly like `FPALBEDONARROW`. The macro pair is decided **per TEXn output** by `vp_fingerprint::texcoord_macro_lanes`, and it keys on the SCALE form only: two pairs that differ merely in which attribute they read (`1B20D02263AA8EE4.vp`, `70931FA705186608.vp`) do **not** qualify, and neither does `498CEDEB9B687381.vp`, whose `0: MUL o[7].xyzw <- v4.xyxy, c[464].xxxx` gives both pairs one scale. Counters: `uv_lane_zw`, `uv_lane_refused` (unit sampled through both pairs -- reported, not judged), `tex_albedo_lane`. Ships **off**: it changes which texture a surface is drawn with, and the RETRYUNSUP precedent says that is not a default. `0` restores round 57 bit-exactly. | |
| `RPCS3_REMIX_UVSCALE2` | **`0` (off)** | **Round 58.** Resolve the two-scale family instead of refusing it. `reduce_moved_attribute`'s MUL arm returns `affine:mul-two-scales` today because `uv_affine_form` carried ONE scale slot; with this on it hands back the second in `scale2_slot`/`scale2_component` and `apply_texcoords` multiplies by both, per lane, in the order the ucode does. Replayed from `bin\remix_ucode\19E83AA14ADE8DF5.vp`: `2: MUL r1.xy <- v5.xyxx, c[464].xxxx` / `11: MUL r1.xyzw <- r1.xyxy, c[463].zzxy` / `13: MOV o[8](TEX1).xy <- r1.xyxx`, with live `c463 = [0.265 0.25 0.5 0]` -- so TEX1.xy = `v5 * c464.x * c463.z`. That unit's albedo (`E2BB0F1275CA632E.fp` `39: TEX R0.xyz <- tc1 [tex1]`, brick `6783D9E9B684BDC8`) is read today from the **heuristic** attribute 1 with the fixed 1/4096, which is the right magnitude by coincidence (`0.5/2048 = 1/4096`) from the wrong attribute. Both constants are read live per draw; nothing is baked, and the finiteness guard covers the product. `two_scales` stays set on the form so the existing `uv_affine_two_scales` census keeps meaning what earlier runs' logs say it means; the apply site distinguishes resolved from refused by `scale2_slot`. Counter: `uv_affine_scale2`. `0` restores the refusal. | |
...
| `RPCS3_REMIX_FPALBEDO` | `1` (on) | Pick the albedo unit from `fp_fingerprint::colour_mask`. `0` picks the lowest referenced, enabled 2D unit with a retry loop free to walk to any other referenced unit -- which is what puts a normal map in the albedo slot. **Round 58:** this is the master switch for **four** rules now, applied in order -- `colour_mask`, then `FPALBEDONARROW`'s channel-count rule, then `FPLERPUNIT`'s blend-weight rule, then `UVLANES`'s macro-lane rule. Every one of them is containment-tested the same way (change nothing unless the elected unit actually differs and the new unit is bindable) and none of them sets `from_ucode`, so the retry policy is identical in all four cases. `0` disables all four. | |
| `RPCS3_REMIX_FPALBEDONARROW` | `1` (on) | Drop units that **cannot carry RGB** -- every sample of them writes fewer than three destination channels, so they are height, gloss, mask or lookup maps -- but only where `colour_mask` saturated (told us nothing) and only where dropping them changes which unit is elected. Structural, not heuristic: the channels are not there. Over `bin\remix_ucode\` it narrows 178 of 202 saturated programs and re-elects **three**, all unit 0 -> unit 1. Counter: `tex_albedo_narrow`. `0` restores the round-42 election bit-exactly. | |
| `RPCS3_REMIX_FPLERPUNIT` | **`0` (off)** | **Round 58.** Drop the **minor** unit of a two-texture lerp the fragment program states outright. Eat Lead's interior programs compute `colour = tex[a] + w*(tex[b] - tex[a])` and this backend elects the lowest sampled unit, which is `tex[a]`; the measured weights say `tex[a]` is 5-15% of the colour. Decoded with the corrected third operand (`docs\remix\fpdis.py`, fixed this round -- it read `SRC2` one bit low and printed a phantom negation on every `MAD` third operand): `B3EFF9C2B4AE9F66.fp` (fp `2dd8807bcbe4e373`, albedo `27D6FC610B1D8589`) `12: TEX R2.xyz <- tc0.zwzz [tex0]` / `10: TEX R0.xyz <- tc0 [tex1]` / `13: ADD R0.xyz <- R0, -R2` / `16: MAD R0.xyz <- R0, c[].x, R2  c=[0.85 ...]`; `4D454157A816F756.fp` 0.95; `89137D5E0D4DFD18.fp` 0.89 (with a per-material tint `34: MUL R0.xyz <- R0, c[0.6196]` on top, which the terminal walk hops through); `5410780A71852F83.fp` 1.15 (with a tint MUL on the major side). **Controls that must not move, and do not:** `0B7312EE9A170A62.fp` is the same shape at `c=[0.35 ...]`, so `w <= 0.5` leaves the mask untouched; `EEDE93C214E686AF.fp` lerps by a *texture* and is refused (`mad-src1`); `633644D3E78A46A4.fp` writes `R0 = tex0` outright and is refused (`terminal-other`). Blast radius MEASURED offline over all 726 `.fp` files with the python mirror (`docs\remix\fpelect.py`, self-tested on those thirteen programs): **31 match the shape, 23 change their election, all 23 move unit 0 -> unit 1, none moves above unit 1**, and the weight distribution is bimodal (0.20-0.49 stay, 0.60-1.20 move) so nothing sits on the boundary. Counter: `tex_albedo_lerp`; census: `Remix albedo-elect:`, whose `would=` field states what the rule WOULD elect on a run with the knob off. Ships **off** -- see `RETRYUNSUP` for why an election widening is not a default. | |
| `RPCS3_REMIX_FORCEALBEDOVP` + `RPCS3_REMIX_FORCEALBEDOFP` + `RPCS3_REMIX_FORCEALBEDOUNIT` | empty / empty / `16` (off) | One measured program-pair override for fragment arithmetic outside the generic terminal walk. Both hashes must match, the unit must be 0-15, and it must remain eligible and bindable; otherwise normal election is unchanged. Eat Lead uses `7CA8A2813F41556B / F4E421CE9C669FE0 / 1` for the picked wall whose inner MAD is 85% tex1 but was still elected as tex0. `Remix albedo-elect:` reports `rule=forced-pair`. | |
...
| `RPCS3_REMIX_TEXBUDGET` | `8` | Cap on `CreateTexture` calls per frame. Anything over budget submits with a null material for that frame and renders untextured until a later frame lets it through. **CORRECTED round 50: setting this to `0` from the environment or a per-game `.conf` is a NO-OP, not "unlimited".** `texture_budget()` (`RemixTextures.cpp:211-219`) is `read_env_u32(name, 0)`, then `env ? env : g_cfg.video.remix.texture_budget`, then `value ? value : umax` — so an env value of 0 is indistinguishable from unset and falls through to the **config** value, which defaults to **8**. Only the *config* value 0 reaches the `umax` branch. To get unlimited from a launcher or a `.conf`, set a large number (e.g. `RPCS3_REMIX_TEXBUDGET=100000`), not `0`. The `bin\BLUS30443.conf` comment "zero is unlimited" is wrong for the same reason. Also **not** read live despite the source comment: it is a `static const`, latched on first call, so it needs a restart. | yes |
...
| `RPCS3_REMIX_SKYEMISSIVEBLEND` | `1` (on) | **The half that gives the sun back (round 13).** Declares `BlendType::kEmissive` on the listed dome's material (`useDrawCallAlphaState=0`, `blendType_hasvalue=1`, `blendType_value=6`), so the runtime takes `calculateAlphaState`'s `!useLegacyAlphaState` arm and reads it directly -- no dependence on blend-factor pattern matching or on `rtx.enableEmissiveBlendModeTranslation`. `alphaState.emissiveBlend` then sets `m_isUnordered` (`rtx_instance_manager.cpp:1216`) and the instance takes `OBJECT_MASK_UNORDERED_ALL_EMISSIVE` (`:1270`), which is deliberately **absent** from `OBJECT_MASK_ALL_STANDARD` -- so the direct shadow ray misses the dome while primary rays still see it. Without this the dome is a closed opaque shell taking `OBJECT_MASK_OPAQUE` (`:1283`) and blocks 100% of the fallback distant sun, which is the entire "the sky lights up the environment instead of the sun" symptom. `0` keeps the glow and keeps the occlusion -- the A/B that attributes any lighting change to this mechanism. | yes |
| `RPCS3_REMIX_SKYEMISSIVEWEXT` | `1` (on) | **Round 53 — the reason the game's own sky dome was not on screen.** Exempts any albedo on the sky-emissive set from `audit_world_extent`'s world-extent refusal, the same early-out `is_sky` and R2's visible backdrop already take (and, like them, the draw is left out of the median sample, so the dome cannot raise the bar for everything else). **MEASURED on Eat Lead (BLUS30267)** in `bin\remix_dump.log`: `Remix skip-census: gate=wext vp=bdc0776911119029 fp=9747c2d02c8abf97 albedo=20703908CDA055F1 extent=1095.06 dist=4.76837e-07 clip=1280x720` — extent 1095 against a frame median of ~0.149 is a ratio of ~7330 into a `STREAKGATE` of 128, and `wext_refused` reaches **7896** on a single `Remix live:` line of that dump. The dome slipped through only in the handful of frames before a median existed, so what was on screen was the runtime's own sky (`rtx.skyMode`), not the game's. The gate's question is *"did this mesh arrive undecoded"*, answered by ratio to the frame median — which is exactly the wrong question to ask of a sky, the one draw whose extent is *supposed* to dwarf the median, and a hash the user typed into `RPCS3_REMIX_SKYEMISSIVE` is a declaration that it is not an undecoded mesh. Keyed on `sky_emissive_albedo_matches()` (**list ∪ classifier-promoted**), not `sky_emissive_listed()`, so the exemption follows exactly the predicate that *builds* the sky material and a hash that gets an emissive material can never then be refused on extent — the stated cost is that if `SKYCLASSIFY` is ever armed at mode 2, a promoted hash is exempt too (tell: `skyclassify_promoted` > 0 alongside `wext_exempt_sky` climbing on a hash not in the conf). **No-op for any title whose sky-emissive set is empty**, i.e. every title but this one today. Counter `wext_exempt_sky` on `Remix live:` and `Remix stats:` — **marginal**, counting only draws this rule *alone* exempted, so it answers "what did this knob change" rather than double-counting the two exemptions that already existed; `0` with a list set means the dome was never drawn in that run (an interior level), not that the knob failed. `0` restores the refusal — the negative control that attributes the sky to this knob. | |
| `RPCS3_REMIX_SKYTAG` | `1` (on) | **Round 53 — measure-only mode for every anchored SKY rule.** `0` lets all four rules that set `is_sky` run unchanged and then withholds the tag. **Why a title would want that, and it is a portability argument rather than a preference:** what a SKY-tagged *API* draw does is fork-dependent by construction — on an aerial-descended runtime (`remix-main+0ff0db1b`) `CameraType::Sky` reaches `DrawCallState` and `rtx_instance_manager` sets `m_isHidden`, so the draw is **deleted**; on numos3 (`6476faea`) `toRtDrawState` clamps Sky→Main and the draw is merely re-cameraed. A title whose sky is made visible by `SKYEMISSIVE` needs no category at all, so there the category is pure fork-dependence with no upside. On Eat Lead the anchored rule does fire — on the *untextured* pass of the second dome program (`vp=487557313fb93949`, `albedo=0`, `wext` 3516–4176 past `SKYEXTENT` 2000). **Implemented as ONE gate where `is_sky` is consumed, not four guards where it is set** (the anchored chain, the learned ring, `SKYBACKDROP=2`, `SKYHASH=2`), and placed *after* the census — so everything above it runs byte for byte as before: each sub-rule's own counter still records what it would have done (`sky_learned_ring`, `skyhash_tagged`, `sky_backdrop_hit`), `m_sky_dome_programs` is still keyed on the would-have result so the learned-ring rule stays armed, and **`Remix sky-census:` still prints `TAGGED`** — with `skytag=0` on the banner that now reads *"would have been tagged"*, which is the one thing a reader of that census has to know. Exactly two things change, the only two consumers of `is_sky`: the SKY category bit at submission, and the world-extent exemption `is_sky` grants. Counter `sky_tag_suppressed` on `Remix live:` and `Remix stats:`, **all four sites folded into the one counter**. `cat_sky` must be `0` whenever `skytag=0`; if it is not, the switch is wired wrong. | |
...
| `RPCS3_REMIX_GUESTLIGHTFIXTUREMAT` | `0` (off) | Replace the visible material of a small, explicitly enrolled fixture that also passes the optional `GUESTLIGHTVP`/`FP` pair with a synthetic warm masked material. This is for packed/procedural bulb shaders whose sampled atlas is not their final RGB; the original albedo hash remains intact for analytical-light triggering. The first successful creation writes `Remix guest-light-fixture-mat:`. | |
...
| `RPCS3_REMIX_GUESTLIGHTCELLS` | `0` (off) | **Round 50, the MOTION gate — never previously documented here.** A guest-light *source* (the `(albedo, vp)` pair that triggered it) is permanently disqualified once it has been seen in this many DISTINCT quantised 0.25-unit cells. Clamped to 64. Where `GUESTLIGHTSTABLE` is a **dwell** test — which an idle NPC passes — this is a **motion** test, which something that walked into the room cannot fake. Measured on Haze at `STABLE=30`: the suit glow card occupied 26 distinct cells, every real fixture exactly 1. Counters `guest_light_moving` (the one frame a source is condemned) and `guest_light_movingrepeat` (every draw it is turned away after). **It measures world motion, and on a title whose `gauge_absent` is 100% of draws that includes BASIS motion — the recovered world position of a bolted-down lamp moves whenever the camera re-bases, so the gate cannot separate a lamp from an NPC there.** Measured on BLUS30267: `CELLS=4` disqualified both enrolled ceiling bulbs at frame 2809, on their fifth cell, four of which were logged before the cut that re-based the scene. Keep it `0` on such a title — with `GUESTLIGHTAUTO=0` and an explicit `GUESTLIGHTALBEDO` list it has nothing to protect anyway. | |
| `RPCS3_REMIX_GUESTLIGHTTRACK` | `0` (off) | **Round 52 — lights that stay on their fixture while the player turns.** A guest light's position is the fixture's local AABB centre pushed through that draw's world transform, and in an un-projection backend that "world" is a **per-frame** quantity: on BLUS30267 the gauge-anchor branches never fire (`gauge_used=0 gauge_prev=0` against `gauge_absent=2055599`, i.e. every draw of every run), so each draw's world is `fused × m_active_camera.reference_inverse` — the model space of whichever draw last won the camera election. Geometry is re-divided every frame; a light was **minted once and never touched again**, so it stayed in whatever basis was current when it was created and every later re-basing left it on a wall. MEASURED, run `pid=29940` frames 1255-1258: the same bulb minted a light at `[17.29 9.106] → [16.68 10.89] → [15.93 12.68] → [15.04 14.46]`, a straight line at ~1.9 units/frame, while the camera on those four frames was identical to five decimals with `cam_age` climbing 0,1,2,3. With this on, a fixture is keyed on `(albedo, vp, quantised LOCAL AABB centre)` — vertex data, which does not depend on the camera at all — and every frame it is drawn its light is re-placed at that frame's recovered position with a **same-hash `CreateLight`**, which the runtime applies in place (`addExternalLight` → `updateLightStaticSleep`). `isDynamic = 1` is then required, not cosmetic: a non-dynamic light stops accepting updates once `isStaticCount` reaches `getNumFramesToPutLightsToSleep()`, which is round 26's sun defect one level down. No `DestroyLight` is ever issued — destroys are deferred to Present and applied *before* same-frame creates with a tombstone, so destroy-then-create in one frame is a **delete**. **With it OFF the drift is still measured**, so the A/B is one conf edit: `guest_light_drift` counts identity re-hits that moved more than 0.25 units and `guest_light_drift_stale` is the subset with `m_camera_age != 0` (a held camera rather than a donor change). Sized by `guest_light_replaced`; `guest_light_track_conflict` must read **0** (non-zero means two instances share one local centre and the identity needs an instance term). Census `Remix guest-light-drift:`, capped 64 per stats window. | |
| `RPCS3_REMIX_GUESTLIGHTMAINCLIP` | `0` (off) | **Round 52 — refuse an AUX-PASS draw of a fixture.** A guest light may only be minted or re-placed from a draw whose surface clip is same / double / half of the session's main clip — the candidate clip gate's own rule, replicated rather than shared. The bulb program on BLUS30267 (`30086232b02bc73a`) also draws into 128x128 targets, and an aux-pass copy divided by the **main** camera's inverse lands at a projectively wrong scale and place; there was no clip gate anywhere on this path. MEASURED: both dump runs carry bulb rows at **2.2-4.1×** the main-pass extent for the same albedo — `extent=1.281` and `1.732` (run `pid=29940` frame 1258), `1.556` and `2.068` (run `pid=20656` frame 2724) — against `0.5` / `0.578` on every main-pass row. Placed **after** the census and sun-card calls so `Remix light-candidate:` and `Remix suncard:` still name a refused aux draw, and **before** the dedup so an aux copy neither stamps an existing light, re-places it, nor mints one. Counter `guest_light_auxclip`; the refusals are named on `Remix guest-light-refused: reason=auxclip` with their clip. Admits everything until a main clip has latched, exactly as the camera gate does. | |
| `RPCS3_REMIX_AUTHOREDLIGHTS` | `0` (off) | **Round 52 - the authored level light list, mode selector.** Eat Lead (BLUS30267) ships a full analytic light list inside its own region containers and `tools/eatlead/extract_lights.py` has extracted all of it into `bin/eatlead_lights/<Level>.lights`: **2,422 records over 10 levels, 2,285 with a world placement**, container parse verified two independent ways over all 63 containers with zero unconsumed tails. Every record carries an **authored colour, cone and intensity**, which is exactly what the derived `GUESTLIGHT*` path structurally cannot - it emits a fixed white `radiance=30` sphere per fixture. `0` = off and byte-identical to a build without the feature; `1` = authored only, which early-returns at the top of `maybe_inject_guest_light()` and counts `guest_light_suppressed` (**not** achieved by blanking `GUESTLIGHTALBEDO`, which would hide the change from the run-start banner and make the A/B unreadable); `2` = both, for the bring-up A/B, where the derived lights serve as render-space ground truth. Submission happens in `flip()` immediately after `submit_camera()`, the same slot and shape as `submit_demons_fire_lights()`. `isDynamic = 1` unconditionally - analytical lights are slept and vanish from LIGHT STATISTICS without it, and dome lights are immune, which disguises the failure as "no lights reach the runtime". `CreateLight` only, on a frame-invariant hash, so every frame is an in-place update; `DestroyLight` runs at teardown and nowhere else. Counters `authored=<loaded>/<placed> submitted culled capped createfail glsuppressed` at the end of `Remix live:`. | |
| `RPCS3_REMIX_AUTHOREDLIGHTLEVEL` | empty (off) | **Round 52.** The level whose `.lights` file to load, e.g. `01_JapaneseRestaurant1`. **Empty means the loader never looks for a file at all**, which is the shipped state, and the load runs *independently of* `AUTHOREDLIGHTS` - so setting only this key loads and reports the file while submitting nothing, which is how the loader is proved on its own. One `Remix authored-lights: level=... map=<16HEX> file=... records=... placed=... declared=... malformed=... classes={...}` line per run, and a missing file prints too. The backend has no notion of which level is loaded (nothing under `Emu/RSX/Remix/` reads `cellFs` or `/dev_bdvd/`); deriving one from the guest's own `USRDIR/Maps/<16HEX>.map` opens needs a hook outside that directory and is deliberately future work. Mission 1 reads `records=398 placed=398`. | |
| `RPCS3_REMIX_AUTHOREDLIGHTAXIS` | `0` (identity) | **Round 52 - a BRING-UP DIAL, not a convention.** `permutation * 8 + signs`, permutations in the order `(x,y,z) (x,z,y) (y,x,z) (y,z,x) (z,x,y) (z,y,x)` and sign bits 1/2/4 negating output component 0/1/2, applied to the file position and to each basis row before the world-to-render map. **`0` is the exact identity** - no permutation, no negation, no arithmetic on the file's numbers. It exists because exactly one bit of the transform is unknown: whether the file's authoring frame (Z-up, right-handed, metres) is the guest's runtime world frame. It is **not** a search space. The earlier "Z is negated" claim is **RETRACTED**: a 1-D vote over the Z offset scored 77/89 for `+Z` against 74/89 for `-Z` (no discriminating power), and a per-frame rigid alignment against the **wrong level** gave `04_BelAirMansion` a perfect 4/4 on a *mission-1* frame while mission 1 never beat the field. With four simultaneous lights at a 0.6 m tolerance the test has no power in either direction, so no axis mapping is entitled to be hardcoded and a "hit" found by sweeping this knob is worth nothing on its own. | |
| `RPCS3_REMIX_AUTHOREDLIGHTOFFSET` | `0,0,0` | **Round 52 - the other bring-up dial.** Signed `x,y,z` (comma / space / semicolon separated) added to the file position **after** the axis permutation and **before** the world-to-render map, i.e. in the *file's* frame. A static world-to-render offset would be the wrong shape of answer on this title and this is not one: measured over 11 of 11 frame pairs, the render space differs **between frames** by a pure translation with no rotation (best case 0.002 units of residual across three independent difference vectors, nine of eleven with `t.z` exactly 0), so any constant fitted in render space is wrong one frame later. Upstream of the per-frame map is where a fixture-to-glow-card offset (expected 0.3-0.5 m along the aim) would actually live. Parsed with `wcstod`, so a component typed with a comma decimal separator reads 0 - the banner prints the parsed values for exactly that reason. | |
| `RPCS3_REMIX_AUTHOREDLIGHTONLY` | `-1` (off) | **Round 52 - the single-light probe.** If `>= 0`, submit **only** this index at **10x radiance**, and exempt it from both the distance cull and the handle cap: a probe that can be silently dropped answers no question at all. One light, one question - is it in the right room? Pair it with `AUTHOREDLIGHTDUMP=1` so the probe's file position, world position and render position are printed on one line in one frame, which makes the answer readable from the log even when nothing visible happens. | |
| `RPCS3_REMIX_AUTHOREDLIGHTDUMP` | `0` (off) | **Round 52.** Emit at most this many `Remix authored-light: idx=... file=[...] world=[...] render=[...] dist=... cls=... rgb=[...] frame=...` rows per frame (clamped to 128, matching `s_max_guest_light_lines`). **Both spaces on one line in one frame is the whole point** - that pairing is the input to the placement measurement and the only thing that makes the residual solvable offline. `world` is post-axis, post-offset, i.e. exactly what was fed to the map, so at the default dials it must equal `file` byte for byte. A separate `Remix authored-map:` line (8 per run, unconditional once lights are submitted) prints the map itself and `identity_l1`, the max abs(M - I) over all sixteen entries - the cheapest available answer to whether `view x projection` is the fused matrix of an identity-model world draw. | |
| `RPCS3_REMIX_AUTHOREDLIGHTMAX` | `64` | **Round 52.** Cap on live authored handles, mirroring `GUESTLIGHTMAX`; survivors are sorted by distance from the eye and the nearest are kept. Clamped to 512, which covers the largest level (`03_Warehouse_Long`, 416 placed). Bypassed while `AUTHOREDLIGHTONLY` is set. Counter `capped`. | |
| `RPCS3_REMIX_AUTHOREDLIGHTDIST` | `40` | **Round 52.** Cull radius in **render** units from the eye. Culling in render space is safe even before the placement transform is settled, because the eye (`m_active_camera.position`, row 3 of `inverse(view)`) and the transformed light positions are in the same space by construction. Bypassed while `AUTHOREDLIGHTONLY` is set. Counter `culled`. | |
| `RPCS3_REMIX_AUTHOREDLIGHTRADIANCE` | `30` | **Round 52.** Global radiance scale: `radiance = (R,G,B)/255 * f0 * this`. Default 30 **because that is the derived path's fixed radiance**, so the first run is directly comparable to it. The file's `f1` (range in metres) is deliberately *not* folded in - Remix spheres fall off physically and have no cutoff, so range has no direct equivalent, and silently reusing it as a second radiance term would make any later calibration unreadable. Alpha is `0xFF` on 2,415 of 2,422 records and `0xD6` on 7; it is ignored. | |
| `RPCS3_REMIX_AUTHOREDLIGHTDIR` | `eatlead_lights` | **Round 52.** Directory under the executable dir holding the `.lights` files, matching where the extractor writes and how every other backend file (`remix_dump.log`, shader dumps, `remix_attachment/`) is located. | |
| `RPCS3_REMIX_AUTHOREDLIGHTMINRADIUS` | `0.2` | **Round 52.** Floor on sphere radius, matching `GUESTLIGHTRADIUS`. The file's `f6` is the source extent and reaches 0.5 and below on `OmniLight`. Class mapping: `OmniLight` (627) to `SphereEXT` with `radius = max(f6, this)` and no shaping; `SpotLight` (631) and `SpotOmniLight` (1942927012) to the same plus `shaping_hasvalue = 1`, `direction` = basis row 1 (**row 1 is the aim**), `coneAngleDegrees = f8 * 0.5` (Remix takes a half angle; `f8` is the full cone and its histogram lands on round degree values); `AreaLight` (746) to `RectEXT` with `xAxis` = row 0, `yAxis` = row 2, `direction` = row 1, `xSize = f6`, `ySize = f7`; `DualAreaLight` (1111553989) to **two** rects, the second under `hash ^ 1` with `direction` and `xAxis` negated, because Remix has no two-sided rect. Rect axes are re-orthonormalised after the map. | |
| `RPCS3_REMIX_AUTHOREDLIGHTCONESOFT` | `0.1` | **Round 52.** `shaping_value.coneSoftness` for the two spot classes, clamped to `[0, 1]`. Parsed signed so an explicit `0` is a real value rather than "unset" - `env_float` refuses everything `<= 0` and would have made `0` a silent no-op. | |
| `RPCS3_REMIX_AUTHOREDLIGHTIGNOREVM` | `1` (on) | **Round 52.** `remixapi_LightInfo::ignoreViewModel`, matching the derived path. The viewmodel is submitted under its own camera; letting world lights into it is a separate decision, and this is the knob that makes it one. | |
...
| `RPCS3_REMIX_UIUVONCE` | `1` (on) | **The UI texcoord scale was being applied TWICE (round 51/53).** `composite_ui_draw`'s `is_demons_menu_draw()` block reads a title's own texcoord scale and bias out of c467 and applies them; **round 52 then added the GENERAL 2D ucode scale immediately below it**, and for Demon's Souls' `f2577d351159c828` both blocks resolve the SAME `c467.z`, so every UV was multiplied by it twice and every inventory icon collapsed onto one texel and rendered as a flat grey block. Replayed ucode (`docs/remix/vpdis.py`): `1: MUL o[7](TEX0).xy <- v8(tc0).xyxx, c[467].zzzz`, against the title-specific block's `demons_uv_scale = packed ? constants[2] : 1` -- the same component, read twice. With this on, the general block stands down wherever the title-specific one already applied a scale or a bias. **The falsifiable half held:** the sibling menu program `fc7a78150c4515d7` writes its texcoord with an `ADD`, so no MUL exists for the scale resolver to find, `ui_uv_ucode_applied` stays false, no second application happens -- and it is the one that rendered correctly. Only the MUL program was broken, which is what a double multiply predicts and a mis-scaled 2D route would not. MEASURED after the fix: `ui_uv_ucode=382662` with `ui_uv_double_skipped=2295972`, and 382662 x 2 triangles x 3 corners = 2,295,972 exactly, so the counters are internally coherent; submitted UV spans went from a uniform ~1e-5 to real atlas rects (`u=[0.60840..0.65918]` for a glyph, `u=[0..1]` for a full-screen blit). Counter `ui_uv_double_skipped` counts triangle CORNERS, not draws. `0` restores the round-52 double application bit-exactly. | |
...
| `RPCS3_REMIX_CLEARBG` | `1` (on) | **The title's framebuffer clear, on frames that submit no world geometry.** This backend never had a `clear_surface` override, so `NV4097_CLEAR_SURFACE` was a no-op and the guest's background colour existed nowhere. On a frame that path-traces something that is invisible -- the scene covers the screen. On a **2D-only** frame (`m_frame_world_draws == 0`: splash, loading screen, full-screen menu) there is no scene at all, so everything the compositor leaves transparent shows the runtime's empty grey. MEASURED on Ratchet & Clank Collection (BCUS98282), which submits geometry on **no** frame in the whole run (`submitted=0`, `skip screen=2748` of 2748 draws): the splash and the menu both sat on a grey field, and the title's own clear is `0xff000000` -- black -- logged once per distinct colour as `Remix: 2D-only frame N, painting the guest clear colour`. With this on, `compositor::underlay` composites the frame's 2D content **over** that colour at submit time and leaves the buffer opaque. Decided at submit, not at `begin_frame`, because `m_frame_world_draws` is only final once the frame's draws are done -- so the background disappears in the *same* frame geometry first arrives, with no one-frame lag either way. Only a **full-channel** colour clear counts (a depth-only or partial-mask clear is not a background), and only the `a8r8g8b8` / `x8r8g8b8_*` surface formats are decoded -- `b8`, `g8b8`, `r5g6b5` and the `b8g8r8` family each re-read the clear register through their own swizzle, and guessing one wrong paints the entire screen the wrong colour, so they are refused by name in a once-per-run notice. Costs one full-screen scalar pass on qualifying frames, counted into `compositor::pixels()` and therefore into the `submit=` term of `Remix timing:`. `0` restores the previous behaviour exactly: no clear is recorded, no underlay runs, and 2D-only frames go back to the runtime background. Tri-state accessor on purpose -- an `env != 0` test against a default-true fallback would have made `CLEARBG=0` a silent no-op. | yes |
| `RPCS3_REMIX_KEEP_UI` | off | Disable the screen-space skip (bisection). | |
| `RPCS3_REMIX_KEEPRT` | off | Composite the title's own render-target blits (its post-process chain) through the CPU rasterizer. Off by default: those draws are not UI, they cost a full-screen fill each, and they are what held one test title at 1.9 FPS. | yes |
...
| `RPCS3_REMIX_UITINTTEXCOORD` | `1` (on) | **Round 52 -- the UI tint the ucode names, and the reason a whole menu came out WHITE AND OPAQUE.** `composite_ui_draw` has only ever read its per-vertex colour from **ATTR3**, the RSX diffuse register. That is the right default and it is not where every title's 2D colour lives; when it is not there, the draw kept the default white tint **at alpha 1**, which paints an opaque slab over everything behind it. MEASURED on Eat Lead (BLUS30267): `Remix ui-biggest: ... inputs=0x7 unit=0 uv=1 tint=FFFFFFFF` against `Remix vtxattr: vp=f2b6988e84056628 ... a0=ub4[len=1.221/1.221/1.221 blen=0.7125 n=40]` -- `len` is the xyz norm of the [0,1]-decoded bytes, so v0 ≈ (0.705, 0.705, 0.705, a) and the title's menu text is authored **light grey**. **The ucode says where the colour is, in two hops that already existed as fields.** The vertex program exports it on a TEXCOORD (`F2B6988E84056628.vp` slice `5: VEC MOV o[8].xyzw <- v0.xyzw`, i.e. TEX1 ← ATTR0) and both fragment programs read it back: `CDFE447F7B7DED21.fp` is `0: MOV R1.xyzw <- TEX1.xyzw ... 5: MOV R0.xyz <- R1.xyzw ... 8: MUL R0.xyzw <- R0.xyzw, C{1,1,1,1} END` (rgb = TEX1.rgb, alpha = coverage × TEX1.w) and `B8E4FC5C9DDDD4E4.fp` is `0: MOV R1 <- TEX1 / 1: MUL R1.xyz <- R1, C{1,1,1,1} / 2: TEX R0 <- tex0 / 3: MUL R0.xyz <- R0, R1` (rgb = tex0.rgb × TEX1.rgb). Two new `fp_out_source` classes, `texcoord_pass` and `texcoord_modulate`, name those shapes and record the TEXn on `fp_fingerprint::out_tint_texcoord`; `vp_fingerprint::texcoord_input[n]` then names the attribute. **`modulate()` reproduces both programs exactly rather than approximately:** for a B8 sheet `sample_bgra` returns white rgb plus coverage in alpha, so `modulate(sample, tint)` = `(TEXn.rgb, coverage × TEXn.a)`, and for the DXT45 draws it is `(tex.rgb × TEXn.rgb, tex.a × TEXn.a)` -- line for line what each program computes. **Three deliberate narrowings.** The classifier's walk steps only through operations that are provably the identity function (an unconditional identity MOV, and a MUL by an inline literal that is exactly 1.0 on the lanes being read -- both programs end in one, which is why a walk that only hops MOVs reports `other`); `vcol_replayable()` is **unchanged**, so the 3D blend-extension replay is byte-identical and `out_vcol_attr` stays 0 on both new classes; and a varying read **directly** by the modulate is deliberately NOT matched, because no decoded program has that shape and this project's record on widening a matcher until something matches is `RETRYUNSUP`. **`0` restores the previous behaviour bit-exactly on BOTH halves** -- the classifier arms are gated on this knob too, so the affected programs fall back into `other` with their round-41 `Remix fpother:` srckind census populated exactly as before, and the compositor reads ATTR3 and nothing else. Counters `ui_tint_tc=<applied>/<unmapped>` and `ui_tint_rgbdrop` on `Remix stats:` and `Remix live:`; banner `uitinttexcoord=`. On a title whose programs are not of these shapes `out_tint_texcoord` stays `0xff` and the knob is a no-op in both positions. | |
| `RPCS3_REMIX_UIUVUCODE` | `1` (on) | **Round 52 -- replay the texcoord scale the program states, on the 2D path.** The compositor computes `u = attribute × uv_scale` with `uv_scale` = 1 unless the texture is `CELL_GCM_TEXTURE_UN`; it has never applied the `texcoord_scale_slot` / `texcoord_affine` forms the 3D path replays for every textured draw (`uv_ucode=678129` on the same runs). Eat Lead's 2D program states one outright: `F2B6988E84056628.vp` slice `0: VEC MUL o[7].xy <- v2.xyxx, c[466].xxxx`, so its TEX0 is the raw ATTR2 scaled by `c[466].x` and the atlas was being sampled with the unscaled attribute. The affine form is preferred over the scalar slot for the same reason `apply_texcoords` prefers it -- it carries a 2×2 and both biases the single divisor cannot express, and it names the attribute it belongs to, so it is refused when the draw reads a different one. Applied **before** the UN texel normalisation, which is the hardware's order: the multiply is in the vertex program and UN is a sampler property. The `one_scale` association and the rows/no-rows split are copied from `apply_texcoords` so the two paths cannot compute different numbers from the same fingerprint. Counters `ui_uv_ucode=<applied>/<refused>`; a slot that cannot be read or a non-finite coefficient keeps the raw attribute and increments the second field rather than producing a NaN UV. **Pre-registered reading: if `c[466].x` is 1.0 this is a measured no-op on this title** -- that is a result, not a failure, and `uvval=` on the new `Remix ui-route:` census prints the live value so it is readable before any pixel moves. `0` restores the previous arithmetic bit-exactly and is the one-relaunch bisect for composited UI that changes size. | |
| `RPCS3_REMIX_UIFORCEVPDW` | `0` (off) | **Round 52 -- the "UI looks world space" half, and it is a ROUTING fault, not a shading one.** `is_screen_space_draw()` admits an orthographic draw only when depth writes are OFF. Eat Lead's menu text is drawn with `depth_write=1` (`Remix kil: vp=f2b6988e84056628 fp=53c93dc604379134 ... depth_write=1 vtx=76`), so it is refused and travels the **world** path -- lit, perspective-divided, clipping against geometry -- and it does so with the compositor ON as well as off, which is why `NOUI=0` alone never fixed the report. `RPCS3_REMIX_UIFORCEVP` could not rescue it either: that override carries the same `!depth_write` term. With this at `1` the **UIFORCEVP override alone** drops that term, for listed programs only. **Why a per-title list and not a classifier change:** the `!depth_write` guard exists so an orthographic depth-writing *world* pass on another title is not mistaken for UI, and it stays; the blast radius here is exactly the programs a conf file already names on the title it names them for, and 0 everywhere else. A ucode rule ("orthographic outer + colour routed through a texcoord + affine-scaled 2D position") is a candidate follow-up, not this round. Counter `ui_forced_dw` -- a strict subset of `ui_forced`, counting only draws admitted **because** of the relaxation -- so it is structurally 0 at the default and any non-zero value is proof the knob is armed. Banner `uiforcevpdw=`. Worthless with `UIFORCEVP` empty, so read the pair together. **Round 59 makes it a MODE and the banner prints the value (0/1/2), not a flag; 0 and 1 are byte-identical to round 52.** `2` = the relaxation admits a listed depth-writing draw **only after the frame's first world draw** (`m_frame_world_draws > 0`). MEASURED on Eat Lead: the DXT1 backdrop `fp=0bde04241aed1c6a` reads `world_before=0` on every `Remix ui-route:` row ever printed (13/13 across the round-52 menu and gameplay censuses, 8/8 again on the 2026-09-10 menu run) while the menu text and HUD read 12..473 -- so the term separates a full-frame background from an overlay without a per-title FP list, and the refused draw is **left on the world path**, not dropped. The term is on the RELAXATION only, never on the plain orthographic classifier: a pre-world term there would push every 2D-only frame's draws (splashes, loading screens) onto the world path on every title. Counter `ui_forced_dw_preworld`, structurally 0 at modes 0/1 and with an empty `UIFORCEVP`. | |
| `RPCS3_REMIX_UIFORCEONLY` | `0` (off) | Composite only draws enrolled by `UIFORCEVP` or the exact `UIFORCEPAIRVP`/`FP` lists. Other automatically classified screen-space draws are skipped instead of entering the overlay. This is for titles such as Eat Lead whose NDC population includes opaque framebuffer copies and construction passes alongside real UI. | |
| `RPCS3_REMIX_UIREFUSEFP` | empty | **Round 59 -- fragment programs that are never UI.** `<fp>[,<fp>...]`, bounded at 8, base 16, parsed by a third deliberate copy of `UIFORCEVP`'s parser (same buffer size, same separators, same "a zero entry is dropped" rule) so three list knobs that mean the same thing cannot have three different failure modes for a mistyped hash. A listed draw is refused **inside `composite_ui_draw`**, after its `Remix ui-route:` census row and after its `UIDUMP` BMP -- so a refused draw is still fully described in the log -- and before the triangle loop and before `++ui_draws`, so `ui_draws` keeps meaning "rasterised". **What it is for:** a full-frame draw the compositor can only paint *over* the finished Remix image -- a background, a "code wall", or a copy of the scene produced by a blit the render-target gate cannot see. Dropping such a draw is exactly what `RPCS3_REMIX_NOUI=1` already does to it (`skip_screen_space`), so a listed program keeps today's picture rather than gaining a new one. **Keyed on the FP, not the albedo:** Eat Lead's code wall rotates its content hash through five values on a ~121-frame cycle and a scene copy's changes every frame, so an albedo list could never hold it. Counter `ui_refused_fp`; census `reason=refusedfp`; list size echoed as `uirefusefps=` on both banners. | |
| `RPCS3_REMIX_TEXREMAP` | `0` (off) | **Round 60 -- the RSX per-texture channel remap (`NV4097_SET_TEXTURE_CONTROL1`, `fragment_texture::decoded_remap()`), which this backend's texture cache has never read.** `decode()` converts guest bytes by FORMAT alone, which is only correct when the remap is identity (`0xAAE4`). With the knob on, a decoded BGRA8 image whose remap is non-identity is rewritten through `control_map`/`channel_map` (tables are indexed **A-R-G-B**, index 0 = alpha). **Measured on Eat Lead (`BLUS30267`), the boot logo sequence:** `vp=f2b6988e84056628 fp=cc1755fe0ee916bf` draws four 1280x720 `fmt=a5` (`A8R8G8B8|LN`) contents whose dumped BMPs are, per texel, `B=255` with `G`, `R` and `A` all equal to one greyscale value -- `91146F0B0628B3A0` is 921600/921600 texels at `B=255 G=0 R=0 A=0`. The fragment program (`bin/remix_ucode/52202C4771A36AAA.fp`) is a plain modulate that samples the texture's **RGB** with no swizzle, so RGB `(luma, luma, 255)` is a full-strength blue field -- the reported defect. Dest BGRA `(255,L,L,L)` back-decodes to guest bytes `[L][L][L][0xFF]`, i.e. **RGBA8 data declared as A8R8G8B8**, which is exactly what a channel remap exists to correct. **`CELL_GCM_TEXTURE_B8` is excluded** -- that arm already hand-writes `(v,v,v,0xFF)`, a spread standing in for this register, and applying the remap on top would double it. The remap joins the content hash **only when actually applied**, so a run with the knob off is bit-for-bit hash-identical to every previous round and existing `bin/remix_tex` replacements keep matching. Counters `tex_remap_seen` / `tex_remap_applied` on `Remix stats:` and `Remix live:`; knob echoed as `texremap=` on both banners; the albedo unit's raw register is on every `Remix ui-route:` row as `remap=`. **Default off** -- it changes what every decoded texture contains, and it has only been A/B'd on one title. | |
...
| `RPCS3_REMIX_UCODESTOREFPHASH` | empty | **Round 52 -- store the ucode of a NAMED fragment program, textured or not.** `<fp>[,<fp>...]`, bounded at 8, base 16, parsed by a straight copy of `UIFORCEVP`'s parser (same buffer size, same separators, same "a zero entry is dropped" rule) so two list knobs that mean the same thing cannot have different failure modes for a mistyped hash. `store_refused_fp_ucode()` was built for **untextured** programs the classifier could not name and is gated accordingly -- `!material` at the call site, `out_rgb_source == other` inside -- which is exactly wrong for a 2D UI program whose shape has to be read offline, because every one of them samples a texture. A listed hash bypasses both gates. It is additionally stored **before the 2D/world split**, and that placement is the point: the composited route returns without ever reaching the untextured store site, so a knob that only relaxed the gates inside the function would still store nothing. File name unchanged (the fp32 export name, i.e. the logged hash xor `0x9e3779b97f4a7c15`), so it lands beside every other program in `bin\remix_ucode\`. Counter: the existing `ucode_fp_stored`; list size echoed as `ucodestorefphashes=` on both banners. Diagnostic only -- refuses nothing, moves no pixel, and costs one set lookup per draw when armed. Needs `RPCS3_REMIX_UCODESTOREFP` (default 1) still on. | |
| `Remix ui-route:` (census, no knob) | always, 64 lines per stats window | **Round 52 -- the UI ROUTING census, from BOTH sides of the screen-space decision.** One line per `(vp, fp, albedo, depth_write, route)` per window, via `dump_line` so it lands in `remix_dump.log` rather than the exclusively locked `RPCS3.log`. `depth_write` and `route` are in the dedup key deliberately: the *same* program pair takes both routes on this title depending on a live register, and a key without them would print whichever side drew first and hide the other -- which is the question the line exists to answer. Fields: the program pair and albedo, `tex=WxH fmt= unit= texaddr=`, `route=2d|world`, `reason=screenarch|identity|ortho|ortho-dw|persp|forced|noouter|noslots` (produced by `screen_space_reason()`, which mirrors `is_screen_space_draw()`'s own order so census and routing cannot disagree, with `forced` latched at the routing site where the `UIFORCEVP`/`UIFORCEPAIR` overrides are visible), the full guest blend state plus `cmask=` and `target=`, `world_before=`, `box=` (post-matrix, in compositor pixels, 2D route only), `tint_attr=` and `tint=[min..max]` over the whole draw, and `uvout= uvslot= uvval=`. **Three of those have never been printed anywhere.** `world_before` is the per-frame count of world draws already submitted, and it is the one fact that separates a background from an overlay -- a frame-covering 2D draw arriving with `world_before=0` was issued *before* the scene, and an overlay compositor that only ever paints on top of the finished Remix image cannot place it. `tint=[min..max]` is a range, not vertex 0's colour, so "every vertex is FFFFFFFF" becomes a measurement instead of an inference. `uvval` is the live `.x` of the constant slot the vertex program multiplies its texcoords by, which settles `UIUVUCODE` before it changes a pixel. **Known gap, stated:** on the `route=world` rows the albedo **content hash is not resolved yet** (the texture bind happens hundreds of lines downstream and running it early would create materials from a diagnostic), so those rows carry the texture's guest address, unit, dimensions and GCM format instead -- which identify it against `Remix tex=` and against this census's own 2D rows. Per-vertex tint work is done only when the budget and window allow a line to be emitted, so a saturated census costs one comparison per draw. | |
...
## Round 59 (2026-09-10) — Eat Lead: the compositor was ALREADY back on; the backdrop was two draws and the census had named both

Two knobs, both shipped **dark**, and one finding that arrived from a conf file rather than from a run.

**`RPCS3_REMIX_UIFORCEVPDW` becomes a mode.** `0` and `1` are byte-identical to round 52 —
`ui_force_vp_depth_write_enabled()` is still `value != 0` and every reader that existed before sees
what it saw. `2` adds a **pre-world** term to the `UIFORCEVP` relaxation and to nothing else: a
listed, depth-writing draw is admitted only once `m_frame_world_draws > 0`. A listed program drawn
*before* the scene keeps the routing it has today — it stays on the world path, it is **not**
dropped. Counter `ui_forced_dw_preworld`.

**`RPCS3_REMIX_UIREFUSEFP=<fp>[,<fp>...]`** refuses a named fragment program inside
`composite_ui_draw`, after its census row and its `UIDUMP` BMP and before the triangle loop, so a
refused draw is fully described and `ui_draws` keeps meaning "rasterised". Counter `ui_refused_fp`,
census `reason=refusedfp`.

**The census evidence both knobs were designed from.** The round-52 `Remix ui-route:` census emits
its `route=world reason=ortho-dw` side at the routing site, which is *not* gated on the compositor,
so every run since 2026-09-06 has been printing the refused-UI population: 41 rows across three
menu-only boots, 446 rows in `run-band57-pid28088.log`, 405 in the run in progress, and **987 rows in
the 2026-09-10 menu run**. They settle three questions that had been open since round 52:

1. **`uvval=1` on every row that names a slot** (all `uvslot=c466`). `c[466].x` is 1.0 on this title,
   so `RPCS3_REMIX_UIUVUCODE` is a measured no-op here. Nothing to do.
2. **No additive UI draw exists.** Every UI row is `blend=1 sfac=770 dfac=771 eq=32774`
   (SRC_ALPHA / ONE_MINUS_SRC_ALPHA / FUNC_ADD) except the DXT1 backdrop (`sfac=1 dfac=0`,
   replace) and the post-process pass (`blend=0`). The planned additive `compositor::blend` arm is
   **dropped on evidence**; `RemixCompositor.*` is untouched.
3. **The backdrop is two different draws.** `fp=0bde04241aed1c6a`, 1280x720, `fmt=a6`
   (`COMPRESSED_DXT1 | LN`), `dw=1`, `sfac=1 dfac=0`, `world_before=0` on **every row of every log**
   (13/13 round 52, 8/8 on 2026-09-10) — a DXT1 texture cannot be a render target, so it is static
   art drawn before the scene. Its ucode (`bin\remix_ucode\95E97D9D65A7607F.fp`) is the
   `texcoord_modulate` shape, so composited it would be an opaque screen-filling wall painted over
   the ray-traced frame. The other, `fp=cc1755fe0ee916bf`, 1280x720 A8R8G8B8 with `dw=0` and five
   rotating content hashes, is the draw round 51 actually composited at `tint=FFFFFFFF`; it did
   **not** appear at all in the 2026-09-10 run.

**What the 2026-09-10 run says about the state that was actually shipped.** `bin\BLUS30267.conf` had
already been moved to `NOUI=0` with `UIFORCEONLY=1` and a seven-entry `UIFORCEPAIRVP2`/`FP2` list, so
the compositor is **on** and the menu text, panels and HUD already read `route=2d reason=forced`
while the DXT1 backdrop still reads `route=world reason=ortho-dw world_before=0`. Mode 2's job is
therefore already being done on this title by FP-exclusion, and mode 2 itself is a **measured no-op
here**: `uiforce_vps=0`, `ui_forced=0`, all 244,049 admissions came through `ui_forced_pair`, and the
pair route reads `ui_force_vp_depth_write_enabled()`, which is still true at 2.

**Two things that run does say, and neither is fixed by this round:**

- **`ui_tint_tc=0/123793`** with `tint_attr=-1 tint=[FFFFFFFF..FFFFFFFF]` on every `route=2d` row.
  The round-52 ATTR0→TEX1 tint chain is not carrying on the pair route at all, so the composited
  text is **white**, not the authored light grey (0.705 ≈ `B4`). `ui_tint_texcoord_unmapped` is the
  refusal counter, so the fragment program named a `TEXn` the vertex program's table has no
  attribute for.
- **`UIFORCEONLY=1` drops every auto-classified 2D draw that is not on the pair list.** Thirteen
  vertex programs are named once each by `Remix: screen-space refusal`, among them a full-frame
  `1280x720 dw=0` quad (`vp=07e29ae10b972579`), a second (`vp=2b03c674980bf216`), two 512x512
  (`27e2f6d206572219`, `af59f2f1a40f69aa`), a 640x360, a 320x180/160x90 bloom chain, and three
  256x720 quads. Those draws reach `++m_stats.skip_screen_space; return;` and are never drawn. This
  is the mechanism behind "the boot logos are not visible" and "the loading screen has lost its
  rotating element and its map background": with the compositor on, `UIFORCEONLY` is what is
  removing them. The architectural answer is `UIFORCEONLY=0` plus a `UIREFUSEFP` list naming the
  full-frame walls — which is what this round built the second knob for — but it is a conf decision
  and a visual check, not a code change.

**MEASURED, 90 s unattended menu run, 2026-09-10 09:35** (`RPCS3.round59-ui-run1.log`, 320 census
rows, `applied=44`, with `UIFORCEVP=f2b6988e84056628` uncommented and `UIFORCEVPDW=2`):

| reading | value | verdict |
| --- | --- | --- |
| `Remix game-config:` | `applied=44 skipped-already-set=0 ignored=0` | PASS |
| banner | `uiforcevpdw=2 uirefusefps=0 ucodestorefphashes=4 uitinttexcoord=1 uiuvucode=1` | PASS |
| `ui_frames` / `ui_draws` | 2456 / 34741 = 14.15 per frame | PASS |
| `ui_forced_dw` / `ui_forced_dw_preworld` | 33941 (13.82/f) / **644** (0.26/f) | the term FIRES; below the pre-registered 0.8-2.5 band because the backdrop is only on some menu screens |
| text on the world path after frame 120 | none; backdrop still `route=world ortho-dw world_before=0` (4 rows) | PASS |
| `Remix kil:` naming `8BD777992BB589A1` after frame 120 | 0 rows | PASS |
| `ui_tint_tc` | **`0/33656`**, `tint_attr=-1` on every `route=2d` row | **FAIL** - the text composites WHITE |
| `ui_rt` / `ui_nospace` / `ui_skipped` | 0 / 0 / 0 | PASS |
| `ui_uv_ucode` | `34741/0`, every row `uvval=1` | PASS |
| `ui` / `ui_px/frame` | 13.55 ms / 907,757 | outside the pre-registered band, but it is menu panels, not a wall - see below |
| `52202C4771A36AAA.fp`, `B2328937CCF2BE39.fp`, `F595B03C3941C27E.fp` | all three on disk | PASS |
| additive rows | none in 320 rows | confirms the dropped 5a arm |

**The `UIREFUSEFP` arming rule as first written is WRONG, and the BMP dump is what caught it.**
Uncommenting `UIFORCEVP` brought `fp=cc1755fe0ee916bf` back onto the compositor: 56 rows,
`route=2d reason=forced`, `world_before=0`, tint alpha `FF`, coverage **1.000**, `blend=0 sfac=1
dfac=0`. By coverage-plus-`world_before` alone that reads as "a full-frame opaque wall, refuse it".
The dumped bitmaps say it is the **boot sequence**: `remix_ui_2321c4a9240e9220_1280x720.bmp` is the
D3Publisher / Vicious Cycle legal-text screen, `remix_ui_cc24fc152293ab9e_1280x720.bmp` the same
text mid-fade, `remix_ui_91146f0b0628b3a0_1280x720.bmp` a solid blue full-frame field. Its census
rows appear only at frames 52-730; the menu census (frames 840-2400) contains **no** full-frame 2D
draw at all, its largest being a 0.375-coverage menu panel. On a **2D-only frame**, `world_before=0`
means "there is no world in this frame", not "this draw is a background over the world" - so the
corrected rule is: a candidate must (a) draw on frames that also contain world geometry, checked by
comparing `frame=` numbers against the menu/HUD rows, and (b) show a scene copy or an opaque slab in
its `UIDUMP` bitmap. `cc1755fe0ee916bf` fails both, so it is **not** listed and the staged conf line
is left empty. Compositing it is what puts the boot logos back on screen.

**Two things this round did not fix, both measured.** `ui_tint_tc=0/33656` - the round-52
ATTR0-to-TEX1 tint chain refuses every draw (`ui_tint_texcoord_unmapped` is the counter climbing),
so the composited text is white rather than the authored `B4` grey. And `RPCS3_REMIX_UIFORCEONLY=1`
still drops thirteen auto-classified vertex programs named once each by `Remix: screen-space
refusal`, including a full-frame `1280x720 dw=0` quad (`07e29ae10b972579`), a second
(`2b03c674980bf216`), two 512x512, a 640x360, a 320x180/160x90 bloom chain and three 256x720 quads -
the population the "loading screen has lost its rotating element" report points at.

## Round 51 (2026-09-06) — Eat Lead: the grey half of the draws was masked out of the guest's own colour buffer

Deployed: `bin\rpcs3.exe` = `bin\rpcs3-next.exe` = md5 **`538CD8175FED08A742E23A04EE61DE9C`**
(was `5B494D30C42CF1BBE05A687579C20729`, preserved as `bin\rpcs3-next-pre-cmask-20260906.exe`).
MSBuild `Release|x64` exit **0**, **0 errors**, no new warnings (the C4723 at
`RemixGSRender.cpp:12987` is pre-existing and moved only because lines were inserted above it).

New knobs: **`RPCS3_REMIX_SKIPCMASK`**, **`RPCS3_REMIX_SKIPNOTARGET`** (both default **on**), rows
in *Materials and textures* above. New census field `cmask=0x%x stencil=%d` on `Remix notex:`.

### The measurement that decided it

`Remix notex:` gained the two registers the whole question turned on, and a gates-off run read them.
For `vp=dc76f9c50a96ccba fp=daf4787c7e54c2f7` — the pair that is 21 of the 22 `no-unit` census rows
and 99.7% of `tex_none` — the census reads:

    clip=1280x720 target=1 depth_write=0 blend=1 cmask=0x0 stencil=1
    clip=512x512  target=0 depth_write=1 blend=0 cmask=0x0 stencil=0

`cmask=0x0` is **every** channel masked off, R G B *and* A. The draw writes nothing to the colour
buffer at all; on the main target it is a stencil pass (`stencil=1`). The discriminating control is
on the same lines: the `class=budget` rows from `vp=f2b6988e84056628` read `cmask=0x7` — RGB on,
alpha off — i.e. ordinary visible geometry. The risk case the plan named (`cmask=0xf stencil=1`,
"it is stencil, not mask") did **not** occur.

The runtime could not have rescued this: the Remix API instance path reads only the **alpha** bit of
`writeMask` (`dxvk-remix\src\dxvk\rtx_render\rtx_instance_manager.cpp`), so a fully masked draw
arrives as a visible grey surface the moment `neutral_material` is attached.

### What the fragment program actually is

`bin\remix_ucode\44C301C5011EBEE2.fp`, 48 bytes, three instructions — reached through the **fp32
export name**, because `store_refused_fp_ucode` strips the `0x9e3779b97f4a7c15` term the log adds:
`daf4787c7e54c2f7 ^ 9e3779b97f4a7c15 = 44C301C5011EBEE2`. Re-checked with a script against the raw
bytes, not read off a hex dump:

    1e7e7e00 c8001c9d c8000001 c8000001   FENCB               (opcode 0x3e, no dest)
    1e010100 00021c9c c8000001 c8000001   MOV R0.xyzw, c.x    (opcode 0x01, mask xyzw, END=1)
    00003f80 00000000 00000000 00000000   inline constant {1.0, 0, 0, 0}

Constant 1.0, `sampled=0x0`, **no texture unit anywhere**. Widening the albedo walk was therefore
never available: there is nothing to walk to, and any texture found would be the wrong one.

### Vertex colour was refuted before a run was spent on it

`vcol_route_blocked=730909` is *exactly* `tex_none (698200) + blend_astranded (32709)` on the same
snapshot — one increment per untextured draw at `apply_vertex_colour`, one per constant-alpha
blended draw at `apply_vertex_alpha`, both refused by `vcol_route_replayable()`. All 32
`Remix vcolroute:` lines this title emitted read `route=none`: no vertex program writes o1/COL0 at
all, so the fpvcol family sitting at `fpclass=0/0/0` is the same fact, not a second one.

### `class=budget` is a correct label, not a mislabel

81 *program pairs* hit `m_textures.over_budget()` at least once, which reads like a starved cache —
but by **draws** it is `tex_none - tex_no_unit ≈ 2130`, matching `tex_deferred=2165`: 0.3% of the
untextured population, almost all on a 128x128 `target=31` (MRT3) offscreen pass. The budget is not
what greys the picture. Its lever, if ever wanted, is `RPCS3_REMIX_TEXBUDGET=100000` in the per-game
conf — never `0`, which is a no-op from env/conf (round 50's correction).

### `pos_input=` printed its pair backwards

Round 50 shipped `pos_input=%llu/%llu` fed `programs, draws` while every document reads it
draws/programs, so `pos_input=33/674496` meant 674,496 draws across 33 programs, not 33 draws. Both
argument lists swapped (`Remix stats:` and `Remix live:`); the format strings are untouched. Read
back off a live line this round: `pos_input=19282/13`.

### Denominator warning

With the gates on, population A never reaches the albedo walk, so `tex_none` and `tex_no_unit` fall.
Measured over the three title-screen runs below, `tex_no_unit` went `4402 -> 0`. **That is a smaller
denominator, not a texture improvement.** Read those counters post-gate, and read `skip_cmask` /
`skip_notarget` beside them.

### Measured, three runs, 90 s each (see the caveat)

| | run A `SKIPUNTEXTUREDVP=dc76…` | run C gates **off** | run B **defaults** |
| --- | --- | --- | --- |
| frames | 886 | 1345 | 1060 |
| `tex_none` / `tex_no_unit` | 1802 / 1795 | 4409 / 4402 | **7 / 0** |
| `notex_mat_applied` (live) | — | 4388 | **0** |
| `skip_cmask` / `skip_notarget` | — | 0 / 0 | **8324 / 13088** |
| `notex_skip` (live) | — | 15 | **0** (exemption works) |
| `cam_resolved` / `cam_fallback` | 173 / 713 | 364 / 981 | 483 / 577 |
| knob echo | — | `skipcmask=0 skipnotarget=0` | `skipcmask=1 skipnotarget=1` |

`notex_skip=0` under the defaults is the check that `cmask` and `notarget` were correctly added to
the `report_skip_census` name exemption: two gates that fire before any albedo is resolved would
otherwise inflate `notex_refused_skip` by ~21k draws.

**CAVEAT — the picture is unverified.** All three runs were unattended, and Eat Lead sits on a
press-start screen forever without a controller input (`cellSysutilGetSystemParamInt(
ID_ENTER_BUTTON_ASSIGN)` polled every frame). The world population these gates were written for is
present at the title screen with the *identical* census shape, which is why the `cmask=` reading
above is trustworthy — but "does the grey field become structure" was **not** measured, and the
in-world magnitudes (218 draws/frame, `cam_contested=2351`) are from the frozen round-50 evidence,
not from these runs. A played session is still owed.

### Demon's Souls relabel, not a regression

`SKIPNOTARGET` fires ahead of `BLUS30443`'s title-gated `skip_camera_surface`, which refuses the
same `surface_color_target()==0` shape. The count moves from `camsurf` to `skip_notarget`;
behaviour is identical. The owed Demon's Souls regression run must not read `camsurf=0` as a loss.


## Round 52 (2026-09-06) — Eat Lead: the 2D compositor's four gaps, three closed and one deferred to a measurement

The report was one sentence: the UI "needs to be actual UI as it looks world space and isn't clean
to read". The compositor was switched **off** (`RPCS3_REMIX_NOUI=1`) and the game's UI hand-tagged
`rtx.worldSpaceUiTextures` as a workaround. Turning the compositor back on does **not** fix it. The
symptom is not one bug; reading the replayed ucode against the compositor's code path shows four
independent gaps, three of which are settled from the ucode alone and one of which is a measurement.

### The ucode, decoded from `bin\remix_ucode\` this round

```
F2B6988E84056628.vp (7 instructions, the menu's 2D vertex program)
 0: VEC MUL o[7].xy   <- v2.xyxx, c[466].xxxx      ; TEX0 = ATTR2 * c[466].x   (a texcoord scale)
 1: VEC MUL r0.xyz    <- v1.xyzx, c[467].xxxx      ; position on ATTR1 (round 50's correction)
 2: VEC MUL r1        <- r0.yyyy, c[257]
 3: VEC MAD r1        <- r0.xxxx, c[256], r1
 4: VEC MAD r0        <- r0.zzzz, c[258], r1
 5: VEC MOV o[8].xyzw <- v0.xyzw                   ; TEX1 = ATTR0 (ub4)  = THE VERTEX COLOUR
 6: VEC ADD o[0].xyzw <- r0.xyzw, c[259].xyzw END  ; HPOS = M(c256..c259) * pos

CDFE447F7B7DED21.fp (the 512x512 B8 glyph atlas 8BD777992BB589A1)
 0: MOV R1.xyzw <- TEX1.xyzw
 1: MUL R0.x    <- R1.wwww, C{1,0,0,0}.xxxx
 2: TEX R0.w    <- TEX0.xyzw tex0                  ; coverage
 3: SLT         <- R0.wwww, C{1/255,0,0,0}.xxxx, R0.xyzw   ; 6: KIL
 4: MUL R0.w    <- R0.xyzw, R0.xxxx                ; alpha = coverage * TEX1.w
 5: MOV R0.xyz  <- R1.xyzw                         ; rgb   = TEX1.xyz
 8: MUL R0.xyzw <- R0.xyzw, C{1,1,1,1}.xyzw  END

B8E4FC5C9DDDD4E4.fp (the 640x480 DXT45 draws)
 0: MOV R1.xyzw <- TEX1.xyzw
 1: MUL R1.xyz  <- R1.xyzw, C{1,1,1,1}.xyzw
 2: TEX R0.xyzw <- TEX0.xyzw tex0
 3: MUL R0.xyz  <- R0.xyzw, R1.xyzw                ; rgb = tex0.rgb * TEX1.rgb
 4: MUL R1.w    <- R1.xyzw, C{1,0,0,0}.xxxx
 5: MUL R1.w    <- R0.xyzw, R1.xyzw                ; alpha = tex0.a * TEX1.a
 8: MUL R0.w    <- R1.xyzw, C{1,1,1,1}.xyzw  END
```

Note what both fragment programs **end** in: a multiply by a literal `{1,1,1,1}`. A classifier that
steps back only through identity MOVs stops on that and reports `other`, which is why neither
program had ever been named. `x * 1.0f == x` for every finite float and both infinities, so
stepping through one cannot change what the program computes — that is the same safety argument
`is_identity_copy` makes and the same step round 42's `col0_chain_scale` already walks.

### The four gaps

1. **Tint.** `composite_ui_draw` read its colour from `map_attribute(3, ...)` and nothing else. This
   title's colour is ATTR0, exported as TEX1 — and `resolve_output_input` refused attribute 0 by the
   same "ATTR0 is the position" premise **round 50 retracted**, so even `texcoord_input[1]` came back
   `0xff`. Result: every UI element white, and every full-screen element at alpha 1, which is the
   opaque wall rather than merely a wrong colour. `RPCS3_REMIX_UITINTTEXCOORD`.
2. **UV scale.** The 2D path never applied `texcoord_scale_slot` / `texcoord_affine`, which the 3D
   path replays for every textured draw. `RPCS3_REMIX_UIUVUCODE`.
3. **Routing.** `is_screen_space_draw()` admits an orthographic draw only when depth writes are off;
   the text draws carry `depth_write=1`, so they go to the **world** path with the compositor on.
   That is the "looks world space" half, and `RPCS3_REMIX_UIFORCEVP` could not rescue it either —
   its override carries the same guard. `RPCS3_REMIX_UIFORCEVPDW`.
4. **Blend and draw order.** `compositor::blend` is straight-alpha source-over, always, and a
   frame-covering 2D draw issued as a *background* can only ever be composited **on top** of the
   Remix image — the overlay model has no "behind the world". **NOT IMPLEMENTED THIS ROUND.**

### What was deliberately NOT built, and what decides it

Gap 4 turns on a fact no log has ever carried: what the 1280x720 backdrop **is**. Its five rotating
content hashes are full FNV hashes of the mip-0 bytes, so identical hash means identical bytes;
static art with a 5-frame cycle fits, and so does a blit-produced render target — and the RT gate
cannot see those by construction, because `m_surface_addresses` is filled only from draw
colour/zeta targets and this backend has no blit hook (`scaled_image_from_memory` is unimplemented).
Three fixes are pre-registered for the three possible answers (refuse a pre-world frame-covering
draw; refuse an opaque depth-tested full-screen quad; register the blit destination). **Choosing one
before the measurement would be guessing**, so nothing was written. The measurement is
`RPCS3_REMIX_UIDUMP` + `UIDUMPVP=f2b6988e84056628` (which writes `bin\remix_ui_<content>_1280x720.bmp`
per distinct backdrop — *look at it*) read together with `world_before=` and `blend=/sfac=/dfac=` on
the new `Remix ui-route:` census. The recipe is written out in `bin\BLUS30267.conf`.

### Round 52 additions

| what | where |
| --- | --- |
| `RPCS3_REMIX_UITINTTEXCOORD`, `RPCS3_REMIX_UIUVUCODE`, `RPCS3_REMIX_UIFORCEVPDW` | `## 2D UI compositor` |
| `RPCS3_REMIX_UCODESTOREFPHASH`, the `Remix ui-route:` census | `## Diagnostics` |
| `fp_out_source::texcoord_pass` / `texcoord_modulate`, `fp_fingerprint::out_tint_texcoord` | `RemixTransforms.h` |
| `resolve_output_input(..., bool allow_attr0)`, passed `position_input != 0 && position_input_enabled()` | `RemixTransforms.cpp` |
| `ui_tint_tc=<applied>/<unmapped> ui_tint_rgbdrop= ui_opaque_skipped= ui_uv_ucode=<applied>/<refused> ui_forced_dw=` | end of the ui group on `Remix stats:` **and** `Remix live:` |
| `uitinttexcoord= uiuvucode= uiforcevpdw= ucodestorefphashes=` | end of the UI group on both banners |

### Expectations, pre-registered

* `ui_tint_tc` ≈ `ui_draws` for `vp=f2b6988e84056628`, with the second field (`unmapped`) at **0**.
* The menu text goes from white to the authored light grey; `Remix ui-biggest:` stops reading
  `tint=FFFFFFFF`.
* `ui_forced_dw > 0` (order of 5/frame in the menu) and **no** `Remix ui-route:` row with
  `vp=f2b6988e84056628 route=world`; no `Remix kil:` row naming `8BD777992BB589A1`.
* Nothing changes on Haze / R2 / Demon's Souls: all three have `position_input == 0`, so the
  `allow_attr0` relaxation is bit-identical there, and none of their fragment programs is of the two
  `texcoord_*` shapes, so `out_tint_texcoord` stays `0xff` and `ui_tint_tc` reads `0/0`.
* If `uvval=` reads **1.0**, gap 2 is a measured no-op on this title. That is the result, not a
  failure; keep the replay, because Haze's `c151` family already proved the divisor is per-program.

### `rtx.conf` — leave the tagging alone until it is verified

A composited draw never reaches Remix as an instance, so a tag on its texture cannot match and
cannot hurt; any draw still on the world path keeps today's look. Only after the counters **and** the
picture are confirmed, with the game closed and `bin\rtx.conf` copied to `bin\rtx.conf.pre-round52`,
remove from `rtx.worldSpaceUiTextures` exactly the hashes the `ui-route` census shows as `route=2d`.
`rtx.uiTextures` is inert on this backend (it is a D3D9-path key) and can stay;
`worldSpaceUiBackgroundTextures = -0xF1A77783A1504379` is none of the five backdrop hashes and is
also left alone.


## Round 50 (2026-09-06) — Eat Lead's white screen: the whole world was decoded from the wrong vertex attribute

Deployed: `bin\rpcs3.exe` = `bin\rpcs3-next.exe` = md5 **`42B2578F856FC797BCC30C75FDE3E6C4`**
(was `7F378E2AA7B85FF30147DA8BE9DD20DF`, preserved as `bin\rpcs3-next-pre-posinput-20260906.exe`).
MSBuild `Release|x64` exit **0**, **0 errors**, one warning: the pre-existing C4723 at
`RemixGSRender.cpp:12834`. **Zero new warnings.**

**IN-GAME VERIFICATION HAS NOT BEEN DONE.** This round was implemented under an explicit instruction
not to launch the emulator. Everything below is offline evidence — replayed ucode, a sweep over the
stored program store, and attribution against `bin\remix_dump.log`. The A/B runs the change is
actually judged on are still outstanding; see "What is still owed" at the end.

### The evidence did not support the hypothesis it was written for

The working hypothesis was "a post-process pass is being classified as UI and painted over the
scene". Two things refute it. The 1280x720 `route=2d` compositor quads from `f2b6988e84056628`
carry the **same four albedo hashes in run B and in run C** (`91146F0B..`, `83D619F8..`,
`1897AC8E..`, `2321C4A9..`) and stop after frame ~490 — that is static loading art, not a per-frame
resolve, and an earlier note claiming "a different albedo hash every frame" was reading two
different runs as one. And the RT-sourced post-process gate the hypothesis asks for already exists
(`RPCS3_REMIX_KEEPRT`, plus the world-path twin) and correctly did not fire, because these textures
are not render targets.

### What the ucode says

Replaying the stored ucode for the Eat Lead programs (`bin\remix_ucode\*.vp`) shows every one of
them feeding HPOS from `v1` or `v2` scaled by a uniform constant, with ATTR0 carrying a
byte-quantised normal routed to a texcoord output:
...
The backend hard-coded ATTR0 as the position at its one decode site, so every Eat Lead mesh reached
Remix as a `[0..254]`-unit normal cloud. Three independent measurements agree: `Remix sky-census:`
`raw=` boxes are integer 0..254 on **42 of 62** rows; the `gate=wext` skip-census extents are median
**250.7** over n=80, which is the diagonal of a 0..254 box; and every `Remix world-refused:` row for
an Eat Lead world program carries `areason=mul-not-attr0`. Thousands of overlapping 254-unit slabs
around the camera is the white screen.

### The fix

`match_const_affine`'s MUL arm records `in.d1.input_src` instead of refusing on it; the index travels
on `vp_fingerprint::position_input` and the one site that fills `m_scratch_vertices` decodes from it.
`composite_ui_draw` consumes the same buffer, so the 2D route is fixed transitively — one changed
argument, not two code paths. Chosen over "refuse and count programs whose position is not ATTR0"
because refusing removes **100%** of Eat Lead's world (every world program is in the family) and a
black screen is not a fix, and over a per-hash special case because 39 programs share the shape.

### The offline sweep, and the one thing it found that the plan did not predict

All **137** stored `.vp` programs were swept (the directory holds 627 files, but 490 of them are
`.fp` fragment programs; a plan figure of "627 programs" counted those too):

| bucket | count |
| --- | --- |
| widened MUL family (an HPOS-chain leaf is `MUL v<K≠0>.xyz * c[S].<bcast>`) | 39 |
| stays ATTR0 (every HPOS-chain input leaf is `v0`) | 52 |
| matches neither | 46 |

Input indices in the widened family: `v2` × 25, `v1` × 8, `v3` × 4, `v7` × 2. Scale slots
c[463]..c[467].

Attributing those 39 hashes against the 735,344-line `bin\remix_dump.log`: **31 are Eat Lead**, 6
never appear in the dump at all, and **2 are Demon's Souls** — `14F25071CB2C399F` (987 occurrences)
and `FEDCEEF2535C262B`. Decoding both shows their `MUL r1.xyzw <- v7.xyzw, c[467].xxxx` is fed
straight into `ARL`, so `v7` there is the **bone index**, not a position; the real position is `v0`,
reaching HPOS through `DPH v0.xyzx, <blended palette row>`. That is precisely the failure mode the
plan's risk register named ("a widened arm matches a program whose `v≠0` MUL is not the position").

Two facts say the arm does not in fact reach it, and both are measurements rather than reasoning
about the grammar:

- Both programs are refused **`areason=no-chain`** in every run recorded in the dump — the walk
  never gets to a matcher at all, let alone to the MUL arm. Their production path is
  `areason=demons` (284 lines for `14F25071CB2C399F`).
- In the **whole** dump the only `mul-*` refusal ever recorded is `mul-not-attr0`, 20 lines, all
  between lines 728874 and 732142, i.e. inside the two Eat Lead run segments. No Demon's Souls run
  has ever produced a `mul-*` refusal; the `areason` values in those segments are only `no-chain`,
  `demons`, `untried` and `chain-reaches-attr`.

This is evidence, not proof — it is bounded by the runs the dump happens to contain. **The first
thing an in-game check must do is launch Demon's Souls and confirm `pos_input=0/0`.**

### What is still owed

- The step-2 baseline / `NOUI=1` A/B (never run: the compositor is exonerated by hash stability and
  by the quads stopping at frame ~490, but that is inference, not a screenshot).
- The step-6 in-game A/B: `pos_input=` climbing, `Remix posinput:` lines reading
  `f2b6988e84056628 input=1 scale_slot=467` and `1b20d02263aa8ee4 input=2 scale_slot=465`,
  `areason=mul-not-attr0` gone, `raw=` boxes no longer integer 0..254, wext extents no longer
  clustered at ~250, and `RPCS3_REMIX_POSINPUT=0` reproducing the white with `pos_input=0/0`.
- A Demon's Souls run confirming `pos_input=0/0` (the regression check above).
- `bin\BLUS30267.conf` exists with **no active knobs**, so a baseline run proves the file is being
  read (`Remix game-config: BLUS30267 applied=0`) without changing behaviour. Both A/B knobs are in
  it as commented-out lines.

### Method notes earned this round

- **A hard-coded index is an assumption even when it is documented.** The comment at the decode site
  correctly said ATTR0 is "the one attribute this backend submits as a position". That is a true
  statement about the backend and it was silently doing duty as a statement about the guest.
- **A per-frame-changing texture hash and a run-stable one look identical if you only read one run.**
  The compositor hypothesis survived as long as it did because two runs were never diffed.
- **When a sweep flags a title that already renders, decode the program before believing either
  outcome.** The two Demon's Souls hits are real matches of the *shape* and would have been a real
  regression — what makes them safe is a separate measurement (`areason=no-chain`), not the shape.
